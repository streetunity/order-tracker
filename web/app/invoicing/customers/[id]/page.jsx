...
export default function CustomerDetailPage() {
  ...
  if (loading) return (
    <><InvoicingNav />
      <div style={{ width: "80%", maxWidth: 1800, margin: "0 auto", paddingTop: 80 }}>
        <div style={{ textAlign: "center", padding: "60px 0", color: "rgba(255,255,255,0.4)" }}>Loading customer...</div>
      </div></>
  );
    ...
    <><InvoicingNav />
      <div style={{ width: "80%", maxWidth: 1800, margin: "0 auto", paddingTop: 80 }}>
        <div style={{ textAlign: "center", padding: "60px 0" }}>
          ...
          <p style={{ color: "rgba(255,255,255,0.5)", marginBottom: 20 }}>Customer not found</p>
          <Link href="/invoicing/customers" style={{ padding: "10px 20px", background: "#dc2626", borderRadius: 8, color: "white", textDecoration: "none", fontSize: 14 }}>Back to Customers</Link>
        </div>
      </div></>
  );
  ...
  return (
    <>
      <InvoicingNav />
      <div style={{ width: "80%", maxWidth: 1800, margin: "0 auto", paddingTop: 80, paddingBottom: 60 }}>

        {/* Back */}
        <button onClick={() => handleNav("/invoicing/customers")} style={{ background: "none", border: "none", color: "rgba(255,255,255,0.35)", cursor: "pointer", fontSize: 13, marginBottom: 16, padding: 0, display: "flex", alignItems: "center", gap: 5 }}>
          ← Back to Customers
        </button>

        {/* ══ HEADER CARD ══════════════════════════════════════════════════ */}
        <div style={{ background: "#1a1a1a", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 14, overflow: "hidden" }}>

          {/* Top row */}
          <div style={{ padding: "20px 28px", display: "flex", gap: 20, alignItems: "center", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
            {/* Avatar */}
            <div style={{ flexShrink: 0, width: 56, height: 56, borderRadius: "50%", background: "linear-gradient(135deg,#dc2626,#991b1b)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20, fontWeight: 700, color: "white", boxShadow: "0 0 0 3px rgba(220,38,38,0.18)", letterSpacing: 1 }}>
              {initials}
            </div>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap", marginBottom: 4 }}>
                <h1 style={{ fontSize: 20, fontWeight: 700, color: "#fff", margin: 0, whiteSpace: "nowrap" }}>{displayName}</h1>
                <button onClick={handleStatusToggle} style={{ padding: "2px 9px", background: statusColor.bg, border: `1px solid ${statusColor.border}`, borderRadius: 20, color: statusColor.text, fontSize: 10, fontWeight: 700, cursor: "pointer", letterSpacing: "0.6px", textTransform: "uppercase" }}>{customer.status}</button>
                {tags.map((t, i) => <span key={i} style={{ padding: "2px 8px", background: "rgba(220,38,38,0.08)", border: "1px solid rgba(220,38,38,0.2)", borderRadius: 12, color: "#dc2626", fontSize: 10 }}>{t}</span>)}
              </div>
              <p style={{ color: "rgba(255,255,255,0.3)", fontSize: 11, fontFamily: "monospace", margin: "0 0 8px" }}>{customer.customerNumber}</p>
              {/* Info pills */}
              <div style={{ display: "flex", gap: 20, flexWrap: "wrap" }}>
                {customer.email && <a href={`mailto:${customer.email}`} style={{ color: "#dc2626", fontSize: 12, textDecoration: "none", display: "flex", alignItems: "center", gap: 5 }}><span style={{ color: "rgba(255,255,255,0.25)" }}>✉</span>{customer.email}</a>}
                {customer.phone && <span style={{ color: "rgba(255,255,255,0.6)", fontSize: 12, display: "flex", alignItems: "center", gap: 5 }}><span style={{ color: "rgba(255,255,255,0.25)" }}>☎</span>{customer.phone}</span>}
                {customer.billingCity && <span style={{ color: "rgba(255,255,255,0.4)", fontSize: 12, display: "flex", alignItems: "center", gap: 5 }}><span style={{ color: "rgba(255,255,255,0.2)" }}>◎</span>{customer.billingCity}{customer.billingState ? `, ${customer.billingState}` : ""}</span>}
                {customer.assignedTo && (
                  <span style={{ color: "rgba(255,255,255,0.5)", fontSize: 12, display: "flex", alignItems: "center", gap: 6 }}>
                    <span style={{ width: 18, height: 18, borderRadius: "50%", background: "linear-gradient(135deg,#dc2626,#991b1b)", display: "inline-flex", alignItems: "center", justifyContent: "center", fontSize: 9, fontWeight: 700, color: "white" }}>{customer.assignedTo.name[0].toUpperCase()}</span>
            ...
            <div style={{ flexShrink: 0, display: "flex", gap: 8 }}>
              <Link href={`/invoicing/estimates/new?customer=${params.id}`} style={{ padding: "8px 14px", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 7, color: "rgba(255,255,255,0.8)", cursor: "pointer", fontSize: 13, textDecoration: "none", whiteSpace: "nowrap" }}>+ Estimate</Link>
              <Link href={`/invoicing/invoices/new?customer=${params.id}`} style={{ padding: "8px 14px", background: "#dc2626", border: "none", borderRadius: 7, color: "white", cursor: "pointer", fontSize: 13, fontWeight: 600, textDecoration: "none", whiteSpace: "nowrap" }}>+ Invoice</Link>
              {!isEditing ? (
                <>
                  <button onClick={() => { setIsEditing(true); setActiveTab("details"); }} style={{ padding: "8px 14px", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 7, color: "rgba(255,255,255,0.8)", cursor: "pointer", fontSize: 13 }}>Edit</button>
                  <button onClick={() => showConfirm("Delete Customer", "Are you sure? This cannot be undone.", handleDelete)} style={{ padding: "8px 14px", background: "rgba(220,38,38,0.07)", border: "1px solid rgba(220,38,38,0.2)", borderRadius: 7, color: "#dc2626", cursor: "pointer", fontSize: 13 }}>Delete</button>
                </>
              ) : (
                <>
                  <button onClick={() => { if (hasUnsavedChanges) { setShowUnsavedModal(true); setPendingNavigation(null); } else { setIsEditing(false); loadCustomer(); } }} style={{ padding: "8px 14px", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 7, color: "rgba(255,255,255,0.8)", cursor: "pointer", fontSize: 13 }}>Cancel</button>
                  <button onClick={handleSave} disabled={saving} style={{ padding: "8px 16px", background: "#dc2626", border: "none", borderRadius: 7, color: "white", cursor: saving ? "not-allowed" : "pointer", fontSize: 13, fontWeight: 600, opacity: saving ? 0.7 : 1 }}>{saving ? "Saving…" : "Save Changes"}</button>
                </>
              )}
            </div>
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(5,1fr)", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
            {[
              ...
              <div key={i} style={{ padding: "12px 20px", borderRight: i < 4 ? "1px solid rgba(255,255,255,0.06)" : "none" }}>
                <div style={{ fontSize: 10, color: "rgba(255,255,255,0.3)", textTransform: "uppercase", letterSpacing: "0.8px", marginBottom: 4 }}>{s.label}</div>
                <div style={{ fontSize: 16, fontWeight: 600, color: s.accent || "rgba(255,255,255,0.85)" }}>{s.value}</div>
              </div>
            ))}
          </div>

          {/* ── Tab bar ──────────────────────────────────────────────────── */}
          <div style={{ display: "flex", padding: "0 28px", background: "#161616" }}>
            {TABS.map(tab => {
              const active = activeTab === tab.id;
              const badge  = tab.id === "transactions" ? txTotalCount : tab.id === "contacts" ? contacts.length : tab.id === "activity" ? activities.length : 0;
              return (
                <button key={tab.id} onClick={() => setActiveTab(tab.id)} style={{ padding: "12px 16px", background: "none", border: "none", borderBottom: active ? "2px solid #dc2626" : "2px solid transparent", color: active ? "#dc2626" : "rgba(255,255,255,0.4)", cursor: "pointer", fontSize: 13, fontWeight: active ? 600 : 400, transition: "all 0.15s", marginBottom: -1, whiteSpace: "nowrap" }}>
                  {tab.label}
                  {badge > 0 && <span style={{ marginLeft: 5, padding: "1px 5px", background: active ? "rgba(220,38,38,0.15)" : "rgba(255,255,255,0.07)", borderRadius: 10, fontSize: 10, color: active ? "#dc2626" : "rgba(255,255,255,0.35)" }}>{badge}</span>}
                </button>
              );
            })}
          </div>
        ...
        {/* Alerts */}
        {error   && <div style={{ padding: "11px 14px", marginTop: 12, background: "rgba(220,38,38,0.07)", border: "1px solid rgba(220,38,38,0.2)", borderRadius: 8, color: "#dc2626", fontSize: 13 }}>{error}</div>}
        {success && <div style={{ padding: "11px 14px", marginTop: 12, background: "rgba(16,185,129,0.07)", border: "1px solid rgba(16,185,129,0.2)", borderRadius: 8, color: "#10b981", fontSize: 13 }}>{success}</div>}
        ...
        <div style={{ marginTop: 1, background: "#1a1a1a", border: "1px solid rgba(255,255,255,0.07)", borderTop: "none", borderRadius: "0 0 14px 14px" }}>
          ...
          {activeTab === "transactions" && (
            <div>
              {/* Filter bar */}
              <div style={{ padding: "14px 24px", borderBottom: "1px solid rgba(255,255,255,0.06)", display: "flex", alignItems: "center", gap: 10 }}>
                {/* Type toggle */}
                {["ALL","ESTIMATE","INVOICE"].map(v => (
                  <button key={v} onClick={() => setTxTypeFilter(v)} style={{ padding: "5px 12px", background: txTypeFilter === v ? "rgba(220,38,38,0.12)" : "rgba(255,255,255,0.04)", border: txTypeFilter === v ? "1px solid rgba(220,38,38,0.3)" : "1px solid rgba(255,255,255,0.08)", borderRadius: 6, color: txTypeFilter === v ? "#dc2626" : "rgba(255,255,255,0.45)", cursor: "pointer", fontSize: 12, fontWeight: txTypeFilter === v ? 600 : 400 }}>
                    {v === "ALL" ? "All Types" : v === "ESTIMATE" ? "Estimates" : "Invoices"}
                  </button>
                ))}
                <div style={{ width: 1, height: 20, background: "rgba(255,255,255,0.08)", margin: "0 4px" }} />
                {/* Status filter */}
                <select value={txStatusFilter} onChange={e => setTxStatusFilter(e.target.value)} style={{ padding: "5px 10px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 6, color: "rgba(255,255,255,0.6)", fontSize: 12, cursor: "pointer" }}>
                ...
                <span style={{ marginLeft: "auto", color: "rgba(255,255,255,0.3)", fontSize: 12 }}>{filteredTx.length} record{filteredTx.length !== 1 ? "s" : ""}</span>
              </div>

              {/* Table header */}
              {filteredTx.length > 0 && (
                <div style={{ display: "grid", gridTemplateColumns: "110px 90px 1fr 110px 110px 90px", gap: 0, padding: "9px 24px", borderBottom: "1px solid rgba(255,255,255,0.05)", background: "rgba(0,0,0,0.15)" }}>
                  {["Date","Type","Number","Amount","Balance Due","Status"].map(h => (
                    <span key={h} style={{ fontSize: 10, fontWeight: 600, color: "rgba(255,255,255,0.3)", textTransform: "uppercase", letterSpacing: "0.7px" }}>{h}</span>
                  ))}
                </div>
              )}
              ...
              {filteredTx.length === 0 ? (
                <div style={{ textAlign: "center", padding: "56px 0" }}>
                  ...
                  <p style={{ color: "rgba(255,255,255,0.35)", fontSize: 14, marginBottom: 18 }}>{transactions.length === 0 ? "No transactions yet for this customer" : "No transactions match the current filters"}</p>
                  <div style={{ display: "flex", gap: 10, justifyContent: "center" }}>
                    <Link href={`/invoicing/estimates/new?customer=${params.id}`} style={{ padding: "9px 18px", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 8, color: "rgba(255,255,255,0.7)", textDecoration: "none", fontSize: 13 }}>New Estimate</Link>
                    <Link href={`/invoicing/invoices/new?customer=${params.id}`} style={{ padding: "9px 18px", background: "#dc2626", border: "none", borderRadius: 8, color: "white", textDecoration: "none", fontSize: 13, fontWeight: 600 }}>New Invoice</Link>
                  </div>
                </div>
              ) : (
                filteredTx.map((tx, idx) => (
                  <div key={tx.id} style={{ display: "grid", gridTemplateColumns: "110px 90px 1fr 110px 110px 90px", gap: 0, padding: "12px 24px", borderBottom: idx < filteredTx.length - 1 ? "1px solid rgba(255,255,255,0.04)" : "none", alignItems: "center", transition: "background 0.1s", cursor: "default" }}
                    onMouseEnter={e => e.currentTarget.style.background = "rgba(255,255,255,0.02)"}
                    onMouseLeave={e => e.currentTarget.style.background = "transparent"}
                  >
                    <span style={{ color: "rgba(255,255,255,0.45)", fontSize: 12 }}>{fmtDate(tx._date)}</span>
                    <span>
                      <span style={{ padding: "2px 8px", background: tx._type === "invoice" ? "rgba(59,130,246,0.1)" : "rgba(139,92,246,0.1)", border: tx._type === "invoice" ? "1px solid rgba(59,130,246,0.25)" : "1px solid rgba(139,92,246,0.25)", borderRadius: 10, color: tx._type === "invoice" ? "#60a5fa" : "#a78bfa", fontSize: 10, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.5px" }}>
                        {tx._type === "invoice" ? "Invoice" : "Estimate"}
                      </span>
                    </span>
                    <span>
                      <Link href={tx._href} style={{ color: "rgba(255,255,255,0.85)", fontSize: 13, fontWeight: 500, textDecoration: "none", fontFamily: "monospace" }}
                    ...
                    <span style={{ color: "rgba(255,255,255,0.8)", fontSize: 13, fontFamily: "monospace" }}>{fmtMoney(tx.total)}</span>
                    <span style={{ color: tx.balanceDue > 0 ? "#f59e0b" : "rgba(255,255,255,0.4)", fontSize: 13, fontFamily: "monospace" }}>{tx._type === "invoice" ? fmtMoney(tx.balanceDue) : "—"}</span>
              ...
              {transactions.length >= 10 && (
                <div style={{ padding: "12px 24px", borderTop: "1px solid rgba(255,255,255,0.05)", textAlign: "center" }}>
                  <span style={{ color: "rgba(255,255,255,0.3)", fontSize: 12 }}>Showing most recent 10 of each type · </span>
                  <Link href={`/invoicing/invoices?customer=${params.id}`} style={{ color: "#dc2626", fontSize: 12, textDecoration: "none" }}>View all invoices</Link>
                </div>
              )}
            </div>
          )}
                  ...
                  {isEditing ? (
                    ...
                    <div style={{ background: "#252525", borderRadius: 9, padding: "12px 14px" }}>
                      {customer.billingAddress
                        ? <div style={{ color: "rgba(255,255,255,0.75)", fontSize: 13, lineHeight: 1.7 }}><div>{customer.billingAddress}</div><div>{customer.billingCity}{customer.billingState ? `, ${customer.billingState}` : ""} {customer.billingZipCode}</div><div style={{ color: "rgba(255,255,255,0.35)", fontSize: 12 }}>{customer.billingCountry}</div></div>
                        : <span style={{ color: "rgba(255,255,255,0.25)", fontSize: 12, fontStyle: "italic" }}>No billing address set</span>}
                    </div>
                  )}
                </div>
                  ...
                  {isEditing ? (
                    ...
                    <div style={{ background: "#252525", borderRadius: 9, padding: "12px 14px" }}>
                      {customer.sameAsBilling
                        ? <span style={{ color: "rgba(255,255,255,0.35)", fontSize: 12, fontStyle: "italic" }}>Same as billing address</span>
                        : customer.shippingAddress
                          ? <div style={{ color: "rgba(255,255,255,0.75)", fontSize: 13, lineHeight: 1.7 }}><div>{customer.shippingAddress}</div><div>{customer.shippingCity}{customer.shippingState ? `, ${customer.shippingState}` : ""} {customer.shippingZipCode}</div><div style={{ color: "rgba(255,255,255,0.35)", fontSize: 12 }}>{customer.shippingCountry}</div></div>
                          : <span style={{ color: "rgba(255,255,255,0.25)", fontSize: 12, fontStyle: "italic" }}>No shipping address set</span>}
                    </div>
                  )}
                </div>
              </div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
                ...
                <div>
                  ...
                  {isEditing ? (
                    ...
                    <div style={{ background: "#252525", borderRadius: 9, padding: "14px" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                        <div style={{ width: 40, height: 40, borderRadius: "50%", background: customer.assignedTo ? "linear-gradient(135deg,#dc2626,#991b1b)" : "rgba(255,255,255,0.07)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14, fontWeight: 700, color: "white" }}>{customer.assignedTo ? customer.assignedTo.name[0].toUpperCase() : "?"}</div>
                        <div>
                          <div style={{ color: "rgba(255,255,255,0.85)", fontWeight: 600, fontSize: 14 }}>{customer.assignedTo?.name || "Unassigned"}</div>
                          {customer.assignedTo?.email && <div style={{ color: "rgba(255,255,255,0.35)", fontSize: 12, marginTop: 2 }}>{customer.assignedTo.email}</div>}
                        </div>
                      </div>
                    </div>
                  )}
                ...
                <div>
                  ...
                  {isEditing ? (
                    ...
                    <div style={{ background: "#252525", borderRadius: 9, padding: "12px 14px", color: "rgba(255,255,255,0.65)", fontSize: 13, whiteSpace: "pre-wrap", minHeight: 52 }}>
          ...
          {activeTab === "activity" && (
            <div style={{ padding: "28px" }}>
              ...
              {activitiesLoading ? (
                <div style={{ textAlign: "center", padding: "40px 0", color: "rgba(255,255,255,0.35)" }}>Loading…</div>
              ) : activities.length === 0 ? (
                <div style={{ textAlign: "center", padding: "40px 0" }}><div style={{ fontSize: 32, marginBottom: 12 }}>📋</div><p style={{ color: "rgba(255,255,255,0.35)", fontSize: 13 }}>No activity recorded yet</p></div>
              ) : (
                <div style={{ position: "relative", paddingLeft: 26 }}>
                  <div style={{ position: "absolute", left: 7, top: 8, bottom: 8, width: 2, background: "rgba(220,38,38,0.12)", borderRadius: 1 }} />
                  {activities.map((a, i) => (
                    <div key={a.id} style={{ position: "relative", marginBottom: i === activities.length - 1 ? 0 : 14 }}>
                      <div style={{ position: "absolute", left: -20, top: 13, width: 10, height: 10, borderRadius: "50%", background: activityColor(a.type), border: "2px solid #1a1a1a", boxShadow: `0 0 0 2px ${activityColor(a.type)}25` }} />
                      <div style={{ background: "#252525", border: "1px solid rgba(255,255,255,0.04)", borderRadius: 9, padding: "12px 14px" }}>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 5 }}>
                          <div style={{ display: "flex", alignItems: "center", gap: 7, flexWrap: "wrap" }}>
                            <span style={{ padding: "2px 8px", background: `${activityColor(a.type)}18`, border: `1px solid ${activityColor(a.type)}30`, borderRadius: 10, color: activityColor(a.type), fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.5px" }}>{activityLabel(a.type)}</span>
                          ...
                          <span style={{ color: "rgba(255,255,255,0.25)", fontSize: 10, whiteSpace: "nowrap", marginLeft: 10 }}>{relativeDate(a.createdAt)}</span>
                        </div>
                        <p style={{ color: "rgba(255,255,255,0.7)", fontSize: 12, margin: 0 }}>{a.description}</p>
                        {a.performedBy && <p style={{ color: "rgba(255,255,255,0.25)", fontSize: 10, marginTop: 4, marginBottom: 0 }}>by {a.performedBy.name}</p>}
                      </div>
                    </div>
                  ))}
                </div>
          ...
          {activeTab === "contacts" && (
            <div style={{ padding: "28px" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
                ...
                <button onClick={() => openContactModal()} style={{ padding: "7px 14px", background: "rgba(220,38,38,0.08)", border: "1px solid rgba(220,38,38,0.25)", borderRadius: 7, color: "#dc2626", cursor: "pointer", fontSize: 12, fontWeight: 500 }}>+ Add Contact</button>
              </div>
              {contacts.length === 0 ? (
                <div style={{ textAlign: "center", padding: "40px 0" }}>
                  <div style={{ fontSize: 32, marginBottom: 12 }}>👤</div>
                  <p style={{ color: "rgba(255,255,255,0.35)", fontSize: 13, marginBottom: 16 }}>No additional contacts</p>
                  <button onClick={() => openContactModal()} style={{ padding: "8px 18px", background: "rgba(220,38,38,0.08)", border: "1px solid rgba(220,38,38,0.25)", borderRadius: 7, color: "#dc2626", cursor: "pointer", fontSize: 12 }}>Add First Contact</button>
                </div>
              ) : (
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(300px,1fr))", gap: 14 }}>
                  {contacts.map(c => (
                    <div key={c.id} style={{ background: "#252525", border: "1px solid rgba(255,255,255,0.05)", borderRadius: 10, padding: 16 }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                          <div style={{ width: 36, height: 36, borderRadius: "50%", background: "rgba(220,38,38,0.1)", border: "1px solid rgba(220,38,38,0.18)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 700, color: "#dc2626" }}>{`${(c.firstName||"?")[0]}${(c.lastName||"")[0]||""}`.toUpperCase()}</div>
                          <div>
                            <div style={{ color: "rgba(255,255,255,0.85)", fontWeight: 600, fontSize: 13 }}>{c.firstName} {c.lastName}</div>
                            <div style={{ display: "flex", gap: 5, marginTop: 3 }}>
                              <span style={{ padding: "1px 6px", background: "rgba(255,255,255,0.05)", borderRadius: 8, color: "rgba(255,255,255,0.4)", fontSize: 9, textTransform: "uppercase" }}>{CONTACT_ROLES.find(r => r.value === c.role)?.label || c.role}</span>
                              {c.isPrimary && <span style={{ padding: "1px 6px", background: "rgba(220,38,38,0.08)", border: "1px solid rgba(220,38,38,0.2)", borderRadius: 8, color: "#dc2626", fontSize: 9, fontWeight: 700, textTransform: "uppercase" }}>Primary</span>}
                            </div>
                          </div>
                        </div>
                        <div style={{ display: "flex", gap: 5 }}>
                          <button onClick={() => openContactModal(c)} style={{ padding: "4px 9px", background: "transparent", border: "1px solid rgba(255,255,255,0.09)", borderRadius: 5, color: "rgba(255,255,255,0.5)", cursor: "pointer", fontSize: 11 }}>Edit</button>
                          <button onClick={() => { setPendingDeleteContactId(c.id); showConfirm("Delete Contact", "Are you sure?", () => handleDeleteContact(c.id)); }} style={{ padding: "4px 9px", background: "transparent", border: "1px solid rgba(220,38,38,0.2)", borderRadius: 5, color: "#dc2626", cursor: "pointer", fontSize: 11 }}>Delete</button>
                        </div>
                      </div>
                      <div style={{ display: "flex", flexDirection: "column", gap: 3 }}>
                        {c.email && <div style={{ fontSize: 12, color: "rgba(255,255,255,0.45)" }}>✉ {c.email}</div>}
                        {c.phone && <div style={{ fontSize: 12, color: "rgba(255,255,255,0.45)" }}>☎ {c.phone}</div>}
                      </div>
                    </div>
                  ))}
                </div>
          ...
          {activeTab === "portal" && (
            <div style={{ padding: "28px", maxWidth: 520 }}>
              ...
              <div style={{ background: "#252525", border: "1px solid rgba(255,255,255,0.05)", borderRadius: 10, padding: 22 }}>
                {customer.portalToken ? (
                  <>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 18, padding: "9px 12px", background: "rgba(16,185,129,0.07)", border: "1px solid rgba(16,185,129,0.18)", borderRadius: 7 }}>
                      <span style={{ color: "#10b981" }}>✓</span><span style={{ color: "#10b981", fontSize: 12, fontWeight: 500 }}>Portal access is active</span>
                    </div>
                    <button onClick={() => setShowPortalUrl(!showPortalUrl)} style={{ padding: "9px 14px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.09)", borderRadius: 7, color: "rgba(255,255,255,0.7)", cursor: "pointer", fontSize: 12, width: "100%", textAlign: "left", display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: showPortalUrl ? 10 : 14 }}>
                      <span>{showPortalUrl ? "Hide URL" : "Show Portal URL"}</span><span style={{ fontSize: 10, color: "rgba(255,255,255,0.25)" }}>{showPortalUrl ? "▲" : "▼"}</span>
                    </button>
                    {showPortalUrl && <div style={{ padding: "10px 12px", background: "rgba(0,0,0,0.25)", borderRadius: 7, marginBottom: 14, wordBreak: "break-all", fontSize: 11, fontFamily: "monospace", color: "rgba(255,255,255,0.5)", border: "1px solid rgba(255,255,255,0.05)" }}>{typeof window !== "undefined" ? `${window.location.origin}/portal/${customer.portalToken}` : `/portal/${customer.portalToken}`}</div>}
                    <button onClick={() => showConfirm("Regenerate Token", "This will invalidate the current portal link. Continue?", handleRegenerateToken)} style={{ padding: "8px 14px", background: "rgba(220,38,38,0.07)", border: "1px solid rgba(220,38,38,0.2)", borderRadius: 7, color: "#dc2626", cursor: "pointer", fontSize: 12 }}>Regenerate Token</button>
                  </>
                ) : (
                  <div style={{ textAlign: "center", padding: "20px 0 16px" }}>
                    <div style={{ fontSize: 32, marginBottom: 12 }}>🔐</div>
                    <p style={{ color: "rgba(255,255,255,0.4)", fontSize: 13, marginBottom: 18 }}>No portal access configured</p>
                    <button onClick={() => showConfirm("Generate Token", "Generate a portal access link for this customer?", handleRegenerateToken)} style={{ padding: "9px 22px", background: "#dc2626", border: "none", borderRadius: 7, color: "white", cursor: "pointer", fontSize: 13, fontWeight: 600 }}>Generate Portal Token</button>
                  </div>
                )}
              </div>
            </div>
