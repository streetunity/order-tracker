...
const REPORT_STAGES = [
  ...
  { key: "TESTING",       label: "Debugging & Testing" },
  { key: "SHIPPING",      label: "Preparing Shipment" },
  { key: "AT_SEA",        label: "Container At Sea" },
  { key: "SMT",           label: "Arrived at SMT" },
...
export default function UnifiedSettingsPage() {
  ...
  // ---- Invoicing Settings (Company + Invoicing tabs) ----
  ...
  const loadStageSettings = async () => {
    ...
    try {
      ...
      if (tRes.ok) { const d = await tRes.json(); setLocalThresh(JSON.parse(JSON.stringify(d))); }
      if (sRes.ok) {
  ...
  const saveCompany = async () => {
    ...
    const body = {}; companyFields.forEach(k => { body[k] = form[k]; });
    // Include rest so PUT doesn't wipe them
    Object.keys(form).forEach(k => { if (!companyFields.includes(k)) body[k] = form[k]; });
    try {
  ...
  const insertVariable = (v) => {
    ...
    const ins = `{{${v}}}`;
    const n = editBody.substring(0, s) + ins + editBody.substring(e);
    setEditBody(n); setTplChanges(true);
            ...
            <>
              ...
              {activeTab === "company" && (
                <>
                  ...
                  <div style={CARD}>
                    <SectionHeader label="Email Branding" desc="Logo shown in email header. Must be a publicly accessible URL. Leave blank to use company name text." />
                    ...
                    {form.logoUrl && (
                      <div style={{ marginTop: 14, padding: "14px 20px", background: "#000", borderRadius: 8, display: "inline-block" }}>
                        <img src={form.logoUrl} alt="Logo preview" style={{ maxHeight: 52, maxWidth: 240, display: "block" }} onError={e => e.target.style.display="none"} />
                      </div>
                    )}
                    <SaveBar hasChanges={companyHasChanges} saving={compSaving} onSave={saveCompany} msg={compMsg} />
                  </div>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 14 }}>
                      {[["invoicePrefix","Invoice Prefix","INV"],["estimatePrefix","Estimate Prefix","EST"],["paymentPrefix","Payment Prefix","PAY"],["customerPrefix","Customer Prefix","CUST"]].map(([k,l,ph]) => (
                        <div key={k}>
                          ...
                          <input style={INP} value={form[k]} onChange={e => setForm(p => ({...p,[k]:e.target.value}))} placeholder={ph} />
                          <p style={HINT}>e.g. {form[k] || ph}-2026-00001</p>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div style={CARD}>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 14 }}>
                      <div>
                        ...
                        <input style={INP} type="number" step="0.01" min="0" value={form.defaultTaxRate} onChange={e => setForm(p => ({...p,defaultTaxRate:e.target.value}))} />
                      ...
                      <div>
                        <label style={LBL}>Estimate Validity (days)</label>
                        <input style={INP} type="number" min="1" value={form.defaultValidityDays} onChange={e => setForm(p => ({...p,defaultValidityDays:e.target.value}))} />
                      </div>
                    </div>
                  </div>
              ...
              {activeTab === "email" && (
                <>
                  ...
                  {emailLoading ? (
                    <div style={{ color: "rgba(255,255,255,0.3)", fontSize: 13, padding: "40px 0" }}>Loading templates\u2026</div>
                  ) : emailView === "templates" ? (
                            ...
                            <div key={cat} style={{ marginBottom: 10 }}>
                              ...
                              {tpls.map(t => (
                                <button key={t.key} onClick={() => selectTemplate(t)} style={{ width: "100%", textAlign: "left", padding: "9px 12px", background: selTpl?.key === t.key ? "rgba(255,255,255,0.06)" : "transparent", border: selTpl?.key === t.key ? "1px solid rgba(255,255,255,0.1)" : "1px solid transparent", borderRadius: 6, cursor: "pointer", color: "rgba(255,255,255,0.8)", marginBottom: 2, fontSize: 13, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                                  {t.name}
                                  {t.isCustomized && <span style={{ fontSize: 9, background: "#dc2626", color: "#fff", padding: "1px 5px", borderRadius: 3, fontWeight: 700, flexShrink: 0 }}>EDITED</span>}
                                </button>
                              ))}
                            </div>
                          ))}
                        ...
                        {selTpl ? (
                          <>
                            <div style={{ padding: "16px 20px", borderBottom: "1px solid rgba(255,255,255,0.06)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                              <div>
                                <div style={{ fontSize: 16, fontWeight: 600 }}>{selTpl.name}</div>
                                {selTpl.description && <div style={{ fontSize: 12, color: "rgba(255,255,255,0.35)", marginTop: 3 }}>{selTpl.description}</div>}
                              </div>
                            ...
                            <div style={{ padding: 20 }}>
                                ...
                                <div style={{ marginBottom: 14, padding: 14, background: "#252525", borderRadius: 7, border: "1px solid rgba(255,255,255,0.07)" }}>
                                  ...
                                  <div style={{ display: "flex", flexWrap: "wrap", gap: 5 }}>
                                    {selTpl.variables.map(v => (
                                      <button key={v.name} onClick={() => insertVariable(v.name)} title={v.description} style={{ padding: "3px 9px", background: "#1a1a1a", border: "1px solid rgba(255,255,255,0.1)", color: "#60a5fa", borderRadius: 4, cursor: "pointer", fontSize: 11, fontFamily: "monospace" }}>&#123;&#123;{v.name}&#125;&#125;</button>
                                    ))}
                                  </div>
                                </div>
                              )}
                              ...
                              <div style={{ marginBottom: 14 }}><label style={LBL}>Closing / Sign-off (HTML) <span style={{ fontWeight: 400, textTransform: "none", letterSpacing: 0 }}>&#8212; optional</span></label><textarea value={editClosing} onChange={e => { setEditClosing(e.target.value); setTplChanges(true); }} style={{ ...INP, minHeight: 80, resize: "vertical", fontFamily: "monospace", fontSize: 12, lineHeight: 1.6 }} /></div>
                              <div><label style={LBL}>Footer (HTML) <span style={{ fontWeight: 400, textTransform: "none", letterSpacing: 0 }}>&#8212; optional</span></label><textarea value={editFooter} onChange={e => { setEditFooter(e.target.value); setTplChanges(true); }} style={{ ...INP, minHeight: 60, resize: "vertical", fontFamily: "monospace", fontSize: 12, lineHeight: 1.6 }} /></div>
                            </div>
                          </>
                        ) : (
                          ...
                          <div key={stage.stage} style={{ ...CARD, marginBottom: 12 }}>
                            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: stage.notify ? 16 : 0 }}>
                              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                                ...
                                <div>
                                  ...
                                  <div style={{ fontSize: 10, color: "rgba(255,255,255,0.3)", fontFamily: "monospace", marginTop: 2 }}>{stage.stage}</div>
                                </div>
                            ...
                            {stage.notify && (
                              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                                <div><label style={LBL}>Subject Line</label><input style={INP} value={stage.subject} onChange={e => setStageConfigs(p => p.map(s => s.stage === stage.stage ? { ...s, subject: e.target.value } : s))} /></div>
                                <div><label style={LBL}>Message Body</label><textarea value={stage.message} onChange={e => setStageConfigs(p => p.map(s => s.stage === stage.stage ? { ...s, message: e.target.value } : s))} style={{ ...INP, minHeight: 60, resize: "vertical", lineHeight: 1.5 }} /></div>
                              </div>
                            )}
                          </div>
                        );
              ...
              {activeTab === "stages" && (
                <>
                  {stagesLoading ? (
                    ...
                    <>
                      <div style={CARD}>
                        <SectionHeader label="Stage Warning Thresholds" desc="Items exceeding Warning days are flagged yellow. Items exceeding Critical days are flagged red." />
                        <div style={{ overflowX: "auto" }}>
                          <table style={{ width: "100%", borderCollapse: "collapse" }}>
                            <thead>
                              <tr style={{ borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
                                {["Stage","Warning Days","Critical Days"].map(h => <th key={h} style={{ padding: "8px 12px", textAlign: "left", fontSize: 11, fontWeight: 700, color: "rgba(255,255,255,0.35)", textTransform: "uppercase", letterSpacing: "0.6px" }}>{h}</th>)}
                              </tr>
                            </thead>
                            <tbody>
                              {REPORT_STAGES.map(rs => {
                                const t = localThresh.find(x => x.stage === rs.key) || { stage: rs.key, warningDays: 0, criticalDays: 0 };
                                return (
                                  <tr key={rs.key} style={{ borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
                                    <td style={{ padding: "10px 12px", fontSize: 13, color: "rgba(255,255,255,0.8)" }}>{rs.label}</td>
                                    <td style={{ padding: "10px 12px" }}>
                                      <input type="number" min="1" value={t.warningDays || ""} onChange={e => { setLocalThresh(p => p.map(x => x.stage === rs.key ? { ...x, warningDays: parseInt(e.target.value) || 0 } : x)); setThreshChanges(true); }} style={{ ...INP, width: 90 }} />
                                    </td>
                                    <td style={{ padding: "10px 12px" }}>
                                      <input type="number" min="1" value={t.criticalDays || ""} onChange={e => { setLocalThresh(p => p.map(x => x.stage === rs.key ? { ...x, criticalDays: parseInt(e.target.value) || 0 } : x)); setThreshChanges(true); }} style={{ ...INP, width: 90 }} />
                                    </td>
                                  </tr>
                                );
                              })}
                      ...
                      <div style={CARD}>
                        <SectionHeader label="Holiday & Extended Shipping" />
                        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 14 }}>
                          <div><label style={LBL}>Season Start (MM-DD)</label><input style={INP} value={holidayStart} onChange={e => { setHolidayStart(e.target.value); setHolidayChanges(true); }} placeholder="10-01" /><p style={HINT}>Holiday season begins</p></div>
                          <div><label style={LBL}>Season End (MM-DD)</label><input style={INP} value={holidayEnd} onChange={e => { setHolidayEnd(e.target.value); setHolidayChanges(true); }} placeholder="12-31" /></div>
                          <div><label style={LBL}>Buffer Days</label><input style={INP} type="number" min="0" max="100" value={bufferDays} onChange={e => { setBufferDays(e.target.value); setHolidayChanges(true); }} /><p style={HINT}>Added to Manufacturing stage Oct&#8211;Dec</p></div>
                          <div><label style={LBL}>Extended Shipping Days</label><input style={INP} type="number" min="0" max="100" value={extendedDays} onChange={e => { setExtendedDays(e.target.value); setHolidayChanges(true); }} /><p style={HINT}>For items with extended lead times</p></div>
                        </div>
                        <SaveBar hasChanges={holidayChanges} saving={holidaySaving} onSave={saveHoliday} msg={holidayMsg} />
                      </div>

                      <div style={CARD}>
                        <SectionHeader label="ETA Management" desc="Recalculate all open order ETAs based on current threshold settings." />
                        <button onClick={recalcETAs} disabled={recalcETA} style={{ padding: "9px 20px", background: recalcETA ? "rgba(245,158,11,0.3)" : "rgba(245,158,11,0.12)", border: "1px solid rgba(245,158,11,0.25)", borderRadius: 7, color: "#f59e0b", fontSize: 13, fontWeight: 600, cursor: recalcETA ? "not-allowed" : "pointer" }}>{recalcETA ? "Recalculating\u2026" : "Recalculate All ETAs"}</button>
                ...
                <>
                  {commLoading ? (
                    ...
                    <>
                      ...
                      {commTab === "global" && (
                        <div style={CARD}>
                          ...
                          <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
                            ...
                            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 14 }}>
                              <div>
                                <label style={LBL}>Default Rate (%)</label>
                                <input style={INP} type="number" step="0.1" min="0" max="100" value={globalComm.defaultRate} onChange={e => { setGlobalComm(p => ({...p,defaultRate:parseFloat(e.target.value)||0})); setGlobalChanges(true); }} />
                              </div>
                              <div>
                                <label style={LBL}>Calculation Basis</label>
                              ...
                              <div>
                                ...
                                <input style={INP} type="number" step="100" min="0" value={globalComm.minimumOrderValue} onChange={e => { setGlobalComm(p => ({...p,minimumOrderValue:parseFloat(e.target.value)||0})); setGlobalChanges(true); }} />
                              </div>
                            </div>
                          </div>
                      ...
                      {commTab === "stages" && (
                        <div style={CARD}>
                          <SectionHeader label="Stage Distribution" desc="Set the percentage of total commission paid when an order reaches each stage. Must total 100%." />
                              ...
                              <tbody>
                                {stageDist.map((item, i) => (
                                  <tr key={i} style={{ borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
                                    <td style={{ padding: "10px 12px" }}>
                                      <select style={{ ...INP, width: 160 }} value={item.stage} onChange={e => { const n = [...stageDist]; n[i].stage = e.target.value; setStageDist(n); setStageDistChange(true); }}>
                                        {COMM_STAGES.map(s => <option key={s} value={s} disabled={stageDist.some((d, j) => d.stage === s && j !== i)}>{s}</option>)}
                                      </select>
                                    </td>
                                ...
                                <div key={rep.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 14px", background: "#252525", borderRadius: 8 }}>
                                  <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                                    ...
                                    <div>
                                      ...
                                      <div style={{ fontSize: 11, color: "rgba(255,255,255,0.35)" }}>{rep.email}</div>
                                    </div>
                                  </div>
                                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
      ...
      {showRecalcModal && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }} onClick={() => { setShowRecalcModal(false); setRecalcReason(""); }}>
          <div style={{ background: "#1a1a1a", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 10, padding: 28, width: "90%", maxWidth: 480 }} onClick={e => e.stopPropagation()}>
            ...
            <p style={{ fontSize: 13, color: "rgba(255,255,255,0.4)", marginBottom: 14 }}>This will recalculate all unpaid commissions based on current rates and stage settings. Commissions with paid payouts will be skipped.</p>
