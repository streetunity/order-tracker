...
function NewEstimateContent() {
  ...
  // Sync local unsaved changes state with global context
  useEffect(() => {
    setGlobalUnsavedChanges(hasUnsavedChanges);
    return () => setGlobalUnsavedChanges(false); // Clean up on unmount
  ...
  useEffect(() => {
    ...
    const handlePopState = (e) => {
      ...
      // Show the warning modal via global context
      globalNavigateWithWarning(document.referrer || '/invoicing/estimates', router);
    };

    // Push current state so we can intercept back
    window.history.pushState(null, '', window.location.href);
  ...
  async function loadTemplate(id) {
    try {
      ...
      if (res.ok) {
        ...
        // Convert template items to estimate items
        if (template.items && template.items.length > 0) {
          const convertedItems = template.items.map(item => {
            // Get price from product, bundle, or custom
            const price = item.product?.price || item.bundle?.price || item.customPrice || 0;
            const cost = item.product?.cost || item.bundle?.cost || null;
  ...
  async function loadCustomers() {
    try {
      ...
      if (res.ok) {
        ...
        setCustomers(data.filter(c => c.status === "ACTIVE" && !c.isDeleted));
      }
    } catch (e) {
  ...
  const discountAmount = discountType === 'PERCENTAGE'
    ...
    : discountType === 'FIXED'
      ...
      : 0;
  const taxableAmount = items.filter(item => item.taxable !== false).reduce((sum, item) => sum + (item.quantity * item.unitPrice), 0);
  ...
  function addBundle(bundle) {
    // Add bundle items as individual line items
    if (bundle.items && bundle.items.length > 0) {
      const componentTotal = bundle.items.reduce((sum, bi) => sum + (bi.product.price * bi.quantity), 0);
      const newItems = bundle.items.map(bi => {
        const productTotal = bi.product.price * bi.quantity;
        const priceRatio = componentTotal > 0 ? productTotal / componentTotal : 0;
        const adjustedPrice = (bundle.price * priceRatio) / bi.quantity;
        return {
          tempId: Date.now() + Math.random(),
    ...
    <>
      ...
      <div style={{ width: "80%", maxWidth: "1800px", margin: "0 auto", paddingTop: 80 }}>
        ...
        <div style={{ marginBottom: 24 }}>
          <button
            type="button"
            onClick={() => navigateWithWarning("/invoicing/estimates")}
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
            <div>
              ...
              {templateName && (
                <p style={{ color: "rgba(255,255,255,0.5)", fontSize: "14px", marginTop: 4 }}>
                  From template: <span style={{ color: "rgba(255,255,255,0.7)" }}>{templateName}</span>
                </p>
              )}
            </div>
        ...
        <form onSubmit={handleSubmit}>
              ...
              {selectedCustomer ? (
                <div style={{
                  ...
                  <div>
                    ...
                    {(selectedCustomer.company || selectedCustomer.companyName) && (
                    ...
                    <div style={{
                      ...
                      {filteredCustomers.map(customer => (
                        ...
                        >
                          ...
                          <div style={{ fontSize: "12px", color: "rgba(255,255,255,0.5)" }}>
                            {customer.company || customer.companyName || customer.email}
                          </div>
                        </div>
                      ))}
                    </div>
          ...
          <div style={sectionStyle}>
              ...
              {showProductDropdown && (filteredProducts.length > 0 || filteredBundles.length > 0) && (
                <div style={{
                  ...
                  {filteredBundles.length > 0 && (
                    <>
                      <div style={{ padding: "8px 14px", fontSize: "11px", color: "rgba(255,255,255,0.4)", textTransform: "uppercase", borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
                        Bundles
                      </div>
                      {filteredBundles.map(bundle => (
                        ...
                        >
                          <div>
                            ...
                            <div style={{ fontSize: "12px", color: "rgba(255,255,255,0.5)" }}>
                              {bundle.itemCount || bundle.items?.length || 0} items
                            </div>
                          </div>
            ...
            {items.length > 0 ? (
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                <thead>
                  <tr style={{ borderBottom: "1px solid rgba(255,255,255,0.1)" }}>
                    ...
                    <th style={{ padding: "8px", textAlign: "right", fontSize: "12px", color: "rgba(255,255,255,0.5)", width: "120px" }}>Total</th>
                      ...
                      <tr key={item.tempId || index} style={{ borderBottom: expandedItems[index] ? "none" : "1px solid rgba(255,255,255,0.05)" }}>
                        <td style={{ padding: "8px", verticalAlign: "top" }}>
                          <button
                            ...
                            title={expandedItems[index] ? "Collapse details" : "Expand details"}
                          >
                            ▶
                          </button>
                        </td>
                        ...
                        <td style={{ padding: "8px", textAlign: "right", fontWeight: "500", color: "rgba(255,255,255,0.9)", verticalAlign: "top", paddingTop: "14px" }}>
                          ...
                          <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "2px" }}>
                            ...
                            <button
                              ...
                              style={{
                                background: "transparent",
                                border: "none",
                                color: index === items.length - 1 ? "rgba(255,255,255,0.2)" : "rgba(255,255,255,0.5)",
              ...
              <div style={{ textAlign: "center", padding: "40px 0", color: "rgba(255,255,255,0.4)" }}>
                No items added yet. Search for products or add a custom item.
              </div>
            )}
          </div>
            ...
            <div style={{
              ...
              {parseFloat(shippingAmount) > 0 && (
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
                  ...
                  <span style={{ color: "rgba(255,255,255,0.9)" }}>{formatCurrency(shippingAmount)}</span>
                </div>
              )}
              <div style={{ display: "flex", justifyContent: "space-between", paddingTop: 8, borderTop: "1px solid rgba(255,255,255,0.1)" }}>
                ...
                <span style={{ fontWeight: "700", color: "#dc2626", fontSize: "18px" }}>{formatCurrency(total)}</span>
              </div>
            </div>
          </div>
          ...
          <div style={sectionStyle}>
            ...
            <div style={{ display: "grid", gap: 16 }}>
              <div>
                <label style={labelStyle}>Customer Notes (visible on estimate)</label>
                <textarea
                  value={notes}
              ...
              <div>
                <label style={labelStyle}>Internal Notes (not visible to customer)</label>
                <textarea
                  value={internalNotes}
        ...
        <div className="modal-overlay" onClick={() => setShowTemplateModal(false)}>
          <div className="modal-content wide" onClick={(e) => e.stopPropagation()}>
            ...
            {templates.length === 0 ? (
              ...
              <div className="modal-list">
                {templates.map(template => (
                  ...
                  >
                    <div className="modal-list-item-title">{template.name}</div>
                    {template.description && (
                      <div className="modal-list-item-description">{template.description}</div>
                    )}
                    <div className="modal-list-item-meta">
                      {template.items?.length || 0} item{(template.items?.length || 0) !== 1 ? 's' : ''}
                      {template.validityDays && ` • Valid for ${template.validityDays} days`}
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="modal-actions">
              <button type="button" className="modal-btn cancel" onClick={() => setShowTemplateModal(false)}>
                Cancel
              </button>
            </div>
          </div>
