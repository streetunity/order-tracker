...
function NewEstimateContent() {
  ...
  useEffect(() => {
    ...
    async function fetchPreset() {
      try {
        ...
        if (res.ok) { const c = await res.json(); setCustomerId(c.id); setSelectedCustomer(c); }
  ...
  async function loadTemplate(id) {
    try {
      ...
      if (res.ok) {
        ...
        setNotes(t.notes || ""); setInternalNotes(t.internalNotes || ""); setTermsConditions(t.termsConditions || "");
        if (t.validityDays) { const d = new Date(); d.setDate(d.getDate() + t.validityDays); setExpiryDate(d.toISOString().split('T')[0]); }
        if (t.items?.length > 0) setItems(t.items.map(item => ({ tempId: Date.now() + Math.random(), productId: item.productId || null, bundleId: item.bundleId || null, name: item.product?.name || item.bundle?.name || item.customName || "", description: item.product?.description || item.bundle?.description || item.customDescription || "", sku: item.product?.sku || item.bundle?.sku || "", quantity: item.quantity || 1, unitPrice: item.product?.price || item.bundle?.price || item.customPrice || 0, unitCost: item.product?.cost || item.bundle?.cost || null, taxable: item.product?.taxable !== false })));
      }
    } catch (e) { console.error(e); }
  }

  async function loadCustomers() {
    try { const r = await fetch("/api/customers", { headers: getAuthHeaders() }); if (r.ok) { const d = await r.json(); setCustomers(d.filter(c => c.status === "ACTIVE" && !c.isDeleted)); } } catch (e) { console.error(e); }
  }
  ...
  async function loadTemplates() {
    try { const r = await fetch("/api/estimate-templates", { headers: getAuthHeaders() }); if (r.ok) { const d = await r.json(); setTemplates(d.filter(t => t.isActive)); } } catch (e) { console.error(e); }
  }

  function applyTemplate(template) {
    setTemplateName(template.name);
    setNotes(template.notes || ""); setInternalNotes(template.internalNotes || ""); setTermsConditions(template.termsConditions || "");
    if (template.validityDays) { const d = new Date(); d.setDate(d.getDate() + template.validityDays); setExpiryDate(d.toISOString().split('T')[0]); }
    if (template.items?.length > 0) setItems(template.items.map(item => ({ tempId: Date.now() + Math.random(), productId: item.productId || null, bundleId: item.bundleId || null, name: item.product?.name || item.bundle?.name || item.customName || "", description: item.product?.description || item.bundle?.description || item.customDescription || "", sku: item.product?.sku || item.bundle?.sku || "", quantity: item.quantity || 1, unitPrice: item.product?.price || item.bundle?.price || item.customPrice || 0, unitCost: item.product?.cost || item.bundle?.cost || null, taxable: item.product?.taxable !== false })));
  ...
  const filteredProducts = products.filter(p => { if (!productSearch) return true; const q = productSearch.toLowerCase(); return p.name?.toLowerCase().includes(q) || p.sku?.toLowerCase().includes(q); });
  const filteredBundles  = bundles.filter(b => { if (!productSearch) return true; return b.name?.toLowerCase().includes(productSearch.toLowerCase()); });
  ...
  async function handleSubmit(e) {
    ...
    try {
      const res = await fetch("/api/estimates", {
        ...
        body: JSON.stringify({ customerId, estimateDate, expiryDate, items: items.map(item => ({ productId: item.productId, fromBundleId: item.fromBundleId, fromBundleName: item.fromBundleName, name: item.name, description: item.description, sku: item.sku, quantity: parseFloat(item.quantity) || 1, unitPrice: parseFloat(item.unitPrice) || 0, unitCost: item.unitCost ? parseFloat(item.unitCost) : null, taxable: item.taxable })), taxRate: parseFloat(taxRate) || 0, discountType: discountType || null, discountValue: discountValue ? parseFloat(discountValue) : null, shippingAmount: parseFloat(shippingAmount) || 0, notes, internalNotes, termsConditions }),
      });
                    ...
                    <>
                      ...
                      {filteredBundles.map(b => (
                        <div key={`b-${b.id}`} onClick={() => addBundle(b)}
                          ...
                          onMouseEnter={(e) => e.currentTarget.style.background = "rgba(255,255,255,0.05)"}
                          onMouseLeave={(e) => e.currentTarget.style.background = "transparent"}>
                          <div><div style={{ fontWeight: "500", color: "rgba(255,255,255,0.9)" }}>{b.name}</div><div style={{ fontSize: "12px", color: "rgba(255,255,255,0.5)" }}>{b.itemCount || b.items?.length || 0} items</div></div>
                    ...
                    <>
                      ...
                      {filteredProducts.map(p => (
                        <div key={`p-${p.id}`} onClick={() => addProduct(p)}
                          ...
                          onMouseEnter={(e) => e.currentTarget.style.background = "rgba(255,255,255,0.05)"}
                          onMouseLeave={(e) => e.currentTarget.style.background = "transparent"}>
                          <div><div style={{ fontWeight: "500", color: "rgba(255,255,255,0.9)" }}>{p.name}</div><div style={{ fontSize: "12px", color: "rgba(255,255,255,0.5)" }}>{p.sku}</div></div>
                              ...
                              <div style={{ background: "rgba(0,0,0,0.2)", borderRadius: "8px", padding: "12px", marginTop: 4 }}>
                                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 12 }}>
                                  ...
                                  <div>
                                    ...
                                    <input type="number" value={item.unitCost || ""} onChange={(e) => updateItem(index, 'unitCost', e.target.value)} style={{ ...inp, padding: "6px 10px", fontSize: 13 }} step="0.01" placeholder="$0.00" draggable={false} />
                                  </div>
                                </div>
                                <div>
          ...
          <div style={sec}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 16, marginBottom: 16 }}>
              <div>
                ...
                <select value={String(taxRate)} onChange={(e) => setTaxRate(parseFloat(e.target.value))} style={inp}>
      ...
      {showTemplateModal && (
        <div className="modal-overlay" onClick={() => setShowTemplateModal(false)}>
          <div className="modal-content wide" onClick={(e) => e.stopPropagation()}>
            ...
            {templates.length === 0 ? <p style={{ color: "#a0a0a0", textAlign: "center", padding: "20px 0" }}>No templates available</p> : (
