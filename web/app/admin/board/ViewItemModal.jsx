...
export function ViewItemModal({ item, order, onClose, onUpdate }) {
  ...
  const handleSave = async () => {
    try {
      ...
      let dataToSend;
      if (isManufacturer) {
        dataToSend = { serialNumber: editedItem.serialNumber };
      } else {
  ...
  const loadCustomerFiles = async () => {
    ...
    try {
      ...
      console.error("Failed to load customer files:", e);
    } finally {
      setLoadingCustomerFiles(false);
    }
  };

  useEffect(() => {
    if (activeTab === "customerDocs") loadCustomerFiles();
  }, [activeTab, order?.id]);
  ...
  useEffect(() => {
    if (activeTab === "shipping" && canManageShipments) loadShipments();
  }, [activeTab]);
  ...
  const handleLinkToShipment = async (shipmentId) => {
    ...
    try {
      ...
      if (!res.ok) { const data = await res.json(); throw new Error(data.error || "Failed to link item"); }
      const shipmentRes = await fetch(`/api/shipments/${shipmentId}`, { headers: getAuthHeaders() });
      if (shipmentRes.ok) {
  ...
  const handleUnlinkShipment = async () => {
    ...
    try {
      ...
      if (!res.ok) { const data = await res.json(); throw new Error(data.error || "Failed to unlink item"); }
  ...
  const handleCreateShipment = async (e) => {
    ...
    try {
      ...
      if (!createRes.ok) { const data = await createRes.json(); throw new Error(data.error || "Failed to create shipment"); }
      ...
      if (!linkRes.ok) { const data = await linkRes.json(); throw new Error(data.error || "Shipment created but failed to link item"); }
    ...
    <>
      <div className="confirm-overlay" onClick={handleClose}>
        <div className="view-item-modal-wide" onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
              ...
              {localItem?.shipmentId && (
                <span style={{
                  ...
                  padding: "4px 8px", backgroundColor: "rgba(220, 38, 38, 0.2)",
                  border: "1px solid #dc2626", borderRadius: "4px", fontSize: "11px", color: "#dc2626"
                }}>
                  <Ship size={12} />
                  Shared Shipment
                </span>
          ...
          {error && (
            <div style={{ padding: "12px", margin: "0 1.5rem", marginTop: "1rem", backgroundColor: "#fef2f2", border: "1px solid #fca5a5", borderRadius: "6px", color: "#dc2626", fontSize: "14px" }}>
              {error}
            </div>
          )}

          {isOrderLocked && !isManufacturer && (
            <div style={{ padding: "12px", margin: "0 1.5rem", marginTop: "1rem", backgroundColor: "#fef3c7", border: "1px solid #fbbf24", borderRadius: "6px", color: "#92400e", fontSize: "13px" }}>
              🔒 <strong>Order is locked.</strong> Only serial number can be edited. Unlock the order to edit other fields.
            </div>
          )}

          {isManufacturer && (
            <div style={{ padding: "12px", margin: "0 1.5rem", marginTop: "1rem", backgroundColor: "#fef2f2", border: "1px solid #fca5a5", borderRadius: "6px", color: "#dc2626", fontSize: "13px" }}>
              ℹ️ <strong>Manufacturer view.</strong> You can edit the serial number and view financial notes.
            </div>
          )}

          {isBroker && (
            <div style={{ padding: "12px", margin: "0 1.5rem", marginTop: "1rem", backgroundColor: "#1f1f1f", border: "1px solid #404040", borderRadius: "6px", color: "#e4e4e4", fontSize: "13px" }}>
          ...
          <div style={{ display: "flex", gap: "8px", margin: "1rem 1.5rem 0", flexWrap: "wrap" }}>
            <button
              ...
              style={{ padding: "8px 16px", background: activeTab === "details" ? "#dc2626" : "#2d2d2d", color: "#fff", border: activeTab === "details" ? "none" : "1px solid #404040", borderRadius: "6px", cursor: "pointer", fontSize: "13px", fontWeight: 500 }}
            >
              Details
            </button>
            <button
              onClick={() => setActiveTab("documents")}
              style={{ padding: "8px 16px", background: activeTab === "documents" ? "#dc2626" : "#2d2d2d", color: "#fff", border: activeTab === "documents" ? "none" : "1px solid #404040", borderRadius: "6px", cursor: "pointer", fontSize: "13px", fontWeight: 500 }}
            >
              Broker Documents
            </button>
            <button
              onClick={() => setActiveTab("customerDocs")}
              style={{ padding: "8px 16px", background: activeTab === "customerDocs" ? "#dc2626" : "#2d2d2d", color: "#fff", border: activeTab === "customerDocs" ? "none" : "1px solid #404040", borderRadius: "6px", cursor: "pointer", fontSize: "13px", fontWeight: 500 }}
            >
              Customer Documents
            </button>
            {canManageShipments && (
              <button
                ...
                style={{ padding: "8px 16px", background: activeTab === "shipping" ? "#dc2626" : "#2d2d2d", color: "#fff", border: activeTab === "shipping" ? "none" : "1px solid #404040", borderRadius: "6px", cursor: "pointer", fontSize: "13px", fontWeight: 500, display: "flex", alignItems: "center", gap: "6px" }}
              >
                <Ship size={14} />
                Shipping
                {localItem?.shipmentId && (
          ...
          <div className="view-item-body-wide">
            ...
            <div className="view-item-left-col">
              ...
              <div className="form-field">
                ...
                <input type="text" value={editedItem.productCode} onChange={(e) => handleInputChange("productCode", e.target.value)} disabled={!canEditField("productCode") || saving} className={canEditField("productCode") ? "" : "field-readonly"} placeholder="e.g., Laser Cutter XL-2000" />
              </div>

              <div className="form-field">
                <label>Model Number</label>
                <input type="text" value={editedItem.modelNumber} onChange={(e) => handleInputChange("modelNumber", e.target.value)} disabled={!canEditField("modelNumber") || saving} className={canEditField("modelNumber") ? "" : "field-readonly"} placeholder="e.g., XL-2000-PRO" />
              </div>
              ...
              <div className="form-field">
                ...
                <input type="text" value={editedItem.voltage} onChange={(e) => handleInputChange("voltage", e.target.value)} disabled={!canEditField("voltage") || saving} className={canEditField("voltage") ? "" : "field-readonly"} placeholder="e.g., 220V or 110V" />
              </div>

              <div className="form-field">
                <label>Power</label>
                <input type="text" value={editedItem.laserWattage} onChange={(e) => handleInputChange("laserWattage", e.target.value)} disabled={!canEditField("laserWattage") || saving} className={canEditField("laserWattage") ? "" : "field-readonly"} placeholder="e.g., 100W" />
              </div>
            </div>
                            ...
                            <div style={{ flex: 1, minWidth: 0 }}>
                              <div style={{ fontSize: "14px", fontWeight: 500, color: "#fff", marginBottom: "4px", wordBreak: "break-word" }}>
                                ...
                                {doc.isShipmentDocument && (
                                  <span style={{ marginLeft: "8px", padding: "1px 6px", backgroundColor: "rgba(220, 38, 38, 0.2)", borderRadius: "3px", fontSize: "10px", color: "#dc2626" }}>Shared</span>
                                )}
                              </div>
          ...
          {activeTab === "customerDocs" && (
            <div style={{ padding: "1.5rem", maxHeight: "60vh", overflowY: "auto" }}>
              {loadingCustomerFiles ? (
                <div style={{ textAlign: "center", color: "#9ca3af", padding: "2rem" }}>Loading customer files...</div>
              ) : (
                <>
                  {/* Total count + link to full manager */}
                  {(() => {
                    const total = CUSTOMER_FILE_CATEGORIES.reduce((n, cat) => n + (customerFiles[cat.id]?.length || 0), 0);
                    return (
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "16px" }}>
                        <span style={{ fontSize: "13px", color: "#9ca3af" }}>
                          {total === 0 ? "No customer files uploaded yet" : `${total} file${total !== 1 ? "s" : ""} uploaded`}
                        </span>
                        <a
                          href={`/admin/orders/${order?.id}/customer-files`}
                          target="_blank"
                  ...
                  {CUSTOMER_FILE_CATEGORIES.map((cat) => {
                    ...
                    return (
                      <div key={cat.id} style={{ marginBottom: "20px" }}>
                          ...
                          <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
                            {files.map((f) => (
                              <a key={f.id} href={f.url} target="_blank" rel="noopener noreferrer" style={{ display: "flex", alignItems: "center", gap: "10px", padding: "10px 12px", backgroundColor: "#1a1a1a", border: "1px solid #2d2d2d", borderRadius: "6px", textDecoration: "none", color: "#e4e4e4", fontSize: "13px" }}>
                                ...
                                <span style={{ flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{f.displayName || f.fileName}</span>
                        ...
                        {(cat.id === "manuals" || cat.id === "documents") && (
                          <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
                            {files.map((f) => (
                              <a key={f.id} href={f.url} target="_blank" rel="noopener noreferrer" style={{ display: "flex", alignItems: "center", gap: "10px", padding: "10px 12px", backgroundColor: "#1a1a1a", border: "1px solid #2d2d2d", borderRadius: "6px", textDecoration: "none", color: "#e4e4e4", fontSize: "13px" }}>
                  ...
                  {/* Empty state */}
                  {CUSTOMER_FILE_CATEGORIES.every(cat => (customerFiles[cat.id]?.length || 0) === 0) && (
                    <div style={{ textAlign: "center", padding: "40px 0", color: "#374151" }}>
                      <div style={{ fontSize: "36px", marginBottom: "10px" }}>📁</div>
                      <p style={{ margin: "0 0 12px", fontSize: "14px" }}>No customer files uploaded yet</p>
                ...
                <div style={{ padding: "20px", backgroundColor: "rgba(220, 38, 38, 0.1)", border: "1px solid rgba(220, 38, 38, 0.3)", borderRadius: "8px" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "16px" }}>
                    <div>
                      ...
                      <p style={{ fontSize: "13px", color: "#e4e4e4", margin: 0 }}>This item is sharing shipping documents with other items in the same container.</p>
                    </div>
                    <button onClick={handleUnlinkShipment} disabled={loadingShipments} style={{ padding: "8px 16px", fontSize: "13px", backgroundColor: "transparent", border: "1px solid #ef4444", color: "#ef4444", borderRadius: "6px", cursor: loadingShipments ? "not-allowed" : "pointer", opacity: loadingShipments ? 0.5 : 1 }}>
                      Unlink
                    </button>
                  </div>
                    ...
                    <div style={{ marginTop: "16px" }}>
                      ...
                      <div style={{ display: "flex", flexWrap: "wrap", gap: "8px" }}>
                        {localItem.shipment.items.filter(i => i.id !== item.id).map(i => (
                          <span key={i.id} style={{ padding: "4px 10px", backgroundColor: "#2d2d2d", border: "1px solid #404040", borderRadius: "4px", fontSize: "12px", color: "#e4e4e4" }}>
                ...
                <div>
                  <div style={{ marginBottom: "20px" }}>
                    ...
                    <p style={{ fontSize: "13px", color: "#9ca3af", margin: 0 }}>
                      Link this item to a shared shipment when multiple items are shipping in the same container. Documents uploaded to the shipment will be shared across all linked items.
                    </p>
                  </div>

                  {shipmentError && (
                    <div style={{ padding: "12px", marginBottom: "16px", backgroundColor: "rgba(239, 68, 68, 0.1)", border: "1px solid rgba(239, 68, 68, 0.3)", borderRadius: "6px", color: "#ef4444", fontSize: "13px" }}>
                      {shipmentError}
                    </div>
                  )}
                  ...
                  {!showCreateShipmentForm ? (
                    <div style={{ padding: "20px", backgroundColor: "#252525", borderRadius: "8px" }}>
                      ...
                      <button onClick={() => setShowCreateShipmentForm(true)} style={{ width: "100%", padding: "12px", fontSize: "14px", backgroundColor: "#dc2626", border: "none", color: "#fff", borderRadius: "6px", cursor: "pointer", fontWeight: 500 }}>
                        + Create New Shipment
                      </button>
                    </div>
                  ) : (
                    <form onSubmit={handleCreateShipment} style={{ padding: "20px", backgroundColor: "#252525", borderRadius: "8px" }}>
                      ...
                      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px", marginBottom: "16px" }}>
                        <div>
                          <label style={{ display: "block", fontSize: "12px", color: "#9ca3af", marginBottom: "6px" }}>Container Number *</label>
                          <input type="text" value={newShipment.containerNumber} onChange={(e) => setNewShipment(s => ({ ...s, containerNumber: e.target.value }))} placeholder="e.g., MSKU1234567" style={{ width: "100%", padding: "10px 12px", fontSize: "14px", backgroundColor: "#1f1f1f", border: "1px solid #404040", borderRadius: "6px", color: "#fff" }} />
                        </div>
                        <div>
                          <label style={{ display: "block", fontSize: "12px", color: "#9ca3af", marginBottom: "6px" }}>Bill of Lading *</label>
                          <input type="text" value={newShipment.billOfLading} onChange={(e) => setNewShipment(s => ({ ...s, billOfLading: e.target.value }))} placeholder="e.g., BOL-2024-ABC" style={{ width: "100%", padding: "10px 12px", fontSize: "14px", backgroundColor: "#1f1f1f", border: "1px solid #404040", borderRadius: "6px", color: "#fff" }} />
                        </div>
                        <div>
                        ...
                        <div>
                          <label style={{ display: "block", fontSize: "12px", color: "#9ca3af", marginBottom: "6px" }}>Vessel Name</label>
                          <input type="text" value={newShipment.vesselName} onChange={(e) => setNewShipment(s => ({ ...s, vesselName: e.target.value }))} placeholder="e.g., Ever Given" style={{ width: "100%", padding: "10px 12px", fontSize: "14px", backgroundColor: "#1f1f1f", border: "1px solid #404040", borderRadius: "6px", color: "#fff" }} />
                        </div>
                      </div>
                      ...
                      <div style={{ display: "flex", gap: "12px", justifyContent: "flex-end" }}>
                        ...
                        <button type="submit" disabled={createShipmentLoading} style={{ padding: "10px 20px", fontSize: "14px", backgroundColor: "#dc2626", border: "none", color: "#fff", borderRadius: "6px", cursor: createShipmentLoading ? "not-allowed" : "pointer", opacity: createShipmentLoading ? 0.7 : 1, fontWeight: 500 }}>
          ...
          <div className="view-item-footer">
            <div style={{ display: "flex", gap: "0.5rem", marginRight: "auto" }}>
              {!isManufacturer && !isBroker && (
                <button onClick={handleLockToggle} disabled={saving || lockingOrder} className="btn-lock" style={{ background: "#dc2626", color: "white", border: "none", padding: "0.5rem 1rem", borderRadius: "6px", cursor: "pointer", fontSize: "14px", transition: "opacity 0.2s" }}>
                  {lockingOrder ? "..." : (isOrderLocked ? "🔓 Unlock Order" : "🔒 Lock Order")}
                </button>
              )}
              {!isManufacturer && !isBroker && !item?.isOrdered && (
                <button onClick={handleMarkOrdered} disabled={saving} className="btn-ordered" style={{ background: "#16a34a", color: "white", border: "none", padding: "0.5rem 1rem", borderRadius: "6px", cursor: "pointer", fontSize: "14px", transition: "opacity 0.2s" }}>
                  {saving ? "..." : "$ Mark as Ordered"}
                </button>
              )}
            </div>

            <button onClick={handleClose} disabled={saving || lockingOrder} className="btn-cancel">
              {hasChanges() ? "Cancel" : "Close"}
            </button>
            {hasChanges() && !isBroker && (
              <button onClick={handleSave} disabled={saving || lockingOrder} className="btn-confirm">
      ...
      {showLockConfirm && (
        <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, backgroundColor: "rgba(0, 0, 0, 0.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1100 }} onClick={() => !lockingOrder && setShowLockConfirm(false)}>
          <div style={{ backgroundColor: "#1f1f1f", border: "1px solid #404040", borderRadius: "8px", padding: "2rem", maxWidth: "500px", width: "90%", boxShadow: "0 4px 20px rgba(0, 0, 0, 0.5)" }} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ padding: "1rem", backgroundColor: "rgba(245, 158, 11, 0.1)", border: "1px solid rgba(245, 158, 11, 0.3)", borderRadius: "6px", marginBottom: "1rem" }}>
              <p style={{ margin: "0 0 0.5rem 0", fontSize: "14px", color: "#f59e0b" }}><strong>What will happen:</strong></p>
            ...
            <div style={{ display: "flex", gap: "1rem", justifyContent: "flex-end", marginTop: "1.5rem" }}>
              <button onClick={() => setShowLockConfirm(false)} disabled={lockingOrder} style={{ background: "#2d2d2d", color: "#fff", border: "1px solid #404040", padding: "0.5rem 1.5rem", borderRadius: "6px", cursor: lockingOrder ? "not-allowed" : "pointer", fontSize: "14px", opacity: lockingOrder ? 0.5 : 1 }}>Cancel</button>
              <button onClick={performLock} disabled={lockingOrder} style={{ backgroundColor: "#dc2626", color: "white", border: "none", padding: "0.5rem 1.5rem", borderRadius: "6px", cursor: lockingOrder ? "not-allowed" : "pointer", fontSize: "14px", opacity: lockingOrder ? 0.5 : 1 }}>
      ...
      {showUnlockDialog && isAdmin && (
        <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, backgroundColor: "rgba(0, 0, 0, 0.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1100 }} onClick={() => !lockingOrder && setShowUnlockDialog(false)}>
          <div style={{ backgroundColor: "#1f1f1f", border: "1px solid #404040", borderRadius: "8px", padding: "2rem", maxWidth: "500px", width: "90%", boxShadow: "0 4px 20px rgba(0, 0, 0, 0.5)" }} onClick={(e) => e.stopPropagation()}>
            <h3 style={{ fontSize: "20px", fontWeight: "600", color: "#fff", marginTop: 0, marginBottom: "1rem" }}>🔓 Unlock Order</h3>
            <p style={{ fontSize: "14px", marginBottom: "1rem", color: "#d1d5db" }}>Please provide a reason for unlocking this order. This will be logged in the audit trail.</p>
            <div style={{ padding: "1rem", backgroundColor: "rgba(245, 158, 11, 0.1)", border: "1px solid rgba(245, 158, 11, 0.3)", borderRadius: "6px", marginBottom: "1rem" }}>
              <p style={{ margin: "0", fontSize: "14px", color: "#f59e0b" }}><strong>Note:</strong> This action will be recorded in the order's audit log.</p>
            </div>
            <p style={{ fontSize: "14px", marginBottom: "0.5rem", color: "#d1d5db" }}><strong>Reason:</strong></p>
            <textarea value={unlockReason} onChange={(e) => setUnlockReason(e.target.value)} placeholder="Enter reason for unlocking (minimum 10 characters)" style={{ width: "100%", minHeight: "100px", padding: "10px", background: "#252525", border: "1px solid #404040", borderRadius: "6px", color: "#fff", fontSize: "14px", marginBottom: "1rem", fontFamily: "inherit", resize: "vertical" }} />
            <div style={{ display: "flex", gap: "1rem", justifyContent: "flex-end", marginTop: "1.5rem" }}>
              <button onClick={() => { setShowUnlockDialog(false); setUnlockReason(""); }} disabled={lockingOrder} style={{ background: "#2d2d2d", color: "#fff", border: "1px solid #404040", padding: "0.5rem 1.5rem", borderRadius: "6px", cursor: lockingOrder ? "not-allowed" : "pointer", fontSize: "14px", opacity: lockingOrder ? 0.5 : 1 }}>Cancel</button>
              <button onClick={performUnlock} disabled={lockingOrder || unlockReason.trim().length < 10} style={{ backgroundColor: "#dc2626", color: "white", border: "none", padding: "0.5rem 1.5rem", borderRadius: "6px", cursor: (lockingOrder || unlockReason.trim().length < 10) ? "not-allowed" : "pointer", fontSize: "14px", opacity: (lockingOrder || unlockReason.trim().length < 10) ? 0.5 : 1 }}>
      ...
      {/* Admin Only Alert */}
      {showAdminOnlyAlert && (
        <div style={{ position: "fixed", top: "100px", right: "24px", backgroundColor: "#1f1f1f", border: "1px solid #404040", borderRadius: "8px", padding: "1rem 1.5rem", boxShadow: "0 4px 20px rgba(0, 0, 0, 0.5)", zIndex: 1200, maxWidth: "400px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
      ...
      {showDeleteConfirm && (
        <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, backgroundColor: "rgba(0, 0, 0, 0.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1100 }} onClick={() => setShowDeleteConfirm(null)}>
          <div style={{ backgroundColor: "#1f1f1f", border: "1px solid #404040", borderRadius: "8px", padding: "2rem", maxWidth: "400px", width: "90%" }} onClick={(e) => e.stopPropagation()}>
            ...
            <p style={{ fontSize: "14px", color: "#d1d5db", marginBottom: "1rem" }}>This action cannot be undone. The document will be permanently deleted.</p>
