...
export default function CommissionsPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      switch (activeTab) {
        case "pending":
          const pendingRes = await fetch("/api/commissions/payouts/pending", { headers, cache: "no-store" });
          if (pendingRes.ok) setPayoutGroups(await pendingRes.json());
          break;
        case "flagged":
          const flaggedRes = await fetch("/api/commissions/flagged", { headers, cache: "no-store" });
          if (flaggedRes.ok) setFlaggedCommissions(await flaggedRes.json());
          break;
        case "approved":
          const approvedRes = await fetch("/api/commissions/approved", { headers, cache: "no-store" });
          if (approvedRes.ok) setApprovedPayouts(await approvedRes.json());
          break;
        case "paid":
          const paidRes = await fetch("/api/commissions/paid?limit=50", { headers, cache: "no-store" });
          if (paidRes.ok) setRecentlyPaid(await paidRes.json());
          break;
        case "orphaned":
          const orphanedRes = await fetch("/api/commissions/orphaned", { headers, cache: "no-store" });
          if (orphanedRes.ok) setOrphanedCommissions(await orphanedRes.json());
          break;
        case "settings":
          if (user.role === "SUPER_ADMIN") router.push("/admin/commission-settings");
          break;
      }
    } catch (error) {
  ...
  const handleApprovePayout = async (payoutId) => {
    try {
      ...
      if (res.ok) {
        setPayoutGroups(prevGroups => prevGroups.map(group => ({
          ...group,
          payouts: group.payouts.filter(p => p.id !== payoutId),
          total: group.payouts.filter(p => p.id !== payoutId).reduce((sum, p) => sum + p.amount, 0)
        })).filter(group => group.payouts.length > 0));
        setSelectedPayouts(prev => { const newSet = new Set(prev); newSet.delete(payoutId); return newSet; });
      } else {
        showNotif(`Failed to approve payout: ${await res.text()}`, "error");
      }
    } catch (error) {
    ...
    try {
      ...
      if (res.ok) {
        setPayoutGroups(prevGroups => prevGroups.map(group => ({
          ...
          total: group.payouts.filter(p => p.id !== rejectPayoutId).reduce((sum, p) => sum + p.amount, 0)
        })).filter(group => group.payouts.length > 0));
        setSelectedPayouts(prev => { const newSet = new Set(prev); newSet.delete(rejectPayoutId); return newSet; });
  ...
  const handleBulkApprove = async () => {
    ...
    try {
      const res = await fetch("/api/commissions/payouts/bulk-approve", {
        method: "POST",
        headers: { "Content-Type": "application/json", ...getAuthHeaders() },
        body: JSON.stringify({ payoutIds: Array.from(selectedPayouts), approvalNotes }),
      });
      if (res.ok) {
        ...
        setPayoutGroups(prevGroups => prevGroups.map(group => ({
          ...
          total: group.payouts.filter(p => !approvedIds.includes(p.id)).reduce((sum, p) => sum + p.amount, 0)
        })).filter(group => group.payouts.length > 0));
        setSelectedPayouts(new Set());
  ...
  const handleUnapprove = (payoutId) => { setUnapprovePayoutId(payoutId); setShowUnapproveModal(true); };
  ...
  const executeUnapprove = async () => {
    try {
      ...
      if (res.ok) {
        ...
        setSelectedPayouts(prev => { const newSet = new Set(prev); newSet.delete(unapprovePayoutId); return newSet; });
  ...
  const handleUnpay = (payoutId) => { setUnpayPayoutId(payoutId); setShowUnpayModal(true); };
  ...
  const handleBulkPay = async () => {
    ...
    try {
      const res = await fetch("/api/commissions/payouts/bulk-pay", {
        method: "POST",
        headers: { "Content-Type": "application/json", ...getAuthHeaders() },
        body: JSON.stringify({ payoutIds: Array.from(selectedPayouts), paymentMethod, paymentNotes }),
      });
      if (res.ok) {
        const result = await res.json();
  ...
  const handleDeleteOrphanedCommission = (commissionId) => { setDeleteOrphanId(commissionId); setShowDeleteOrphanModal(true); };
  ...
  const generatePdfReport = async () => {
    ...
    try {
      ...
      const tableData = payouts.map((payout) => {
        ...
        const appliedCommissionPercent = stageSettings.length > 0 ? (commissionRate / stageSettings.length).toFixed(2) : commissionRate.toFixed(2);
        return [
      ...
      if (signatureY + 40 > pageHeight - 20) { doc.addPage(); addSignatureSection(doc, 30, pageWidth); }
      else { addSignatureSection(doc, signatureY, pageWidth); }
  ...
  const generateReportFromSelected = async () => {
    ...
    try {
      const selectedItems = recentlyPaid.filter(p => selectedPayouts.has(p.id));
      if (selectedItems.length === 0) { showNotif("No selected commissions found", "error"); return; }
      ...
      const uniqueSalesReps = [...new Set(selectedItems.map(p => p.itemCommission?.commission?.salesPersonName).filter(Boolean))];
      const salesRepText = uniqueSalesReps.length === 1 ? uniqueSalesReps[0] : `${uniqueSalesReps.length} Sales Reps`;
      const paidDates = selectedItems.map(p => new Date(p.paidAt)).sort((a, b) => a - b);
      const minDate = paidDates[0];
      const maxDate = paidDates[paidDates.length - 1];

      doc.setFontSize(12);
      doc.setFont(undefined, "normal");
      doc.text(`Sales Agent: ${salesRepText}`, 14, 35);
      doc.text(`Pay Period: ${minDate.toLocaleDateString()} - ${maxDate.toLocaleDateString()}`, 14, 42);
      doc.text(`Report Generated: ${new Date().toLocaleDateString()}`, 14, 49);
      doc.text(`Selected Items: ${selectedItems.length}`, 14, 56);

      const totalPaid = selectedItems.reduce((sum, p) => sum + parseFloat(p.amount || 0), 0);
      const tableData = selectedItems.map((payout) => {
        const paymentNumber = stageSettings.findIndex(s => s.stage === payout.stage) + 1;
        const commissionRate = payout.itemCommission?.commission?.commissionRate || 0;
        const appliedCommissionPercent = stageSettings.length > 0 ? (commissionRate / stageSettings.length).toFixed(2) : commissionRate.toFixed(2);
        return [
          payout.itemCommission?.commission?.order?.account?.name || "N/A",
          payout.itemCommission?.productCode || "N/A",
          payout.itemCommission?.commission?.salesPersonName || "N/A",
          paymentNumber > 0 ? toOrdinal(paymentNumber) : "N/A",
          `$${parseFloat(payout.amount || 0).toFixed(2)}`,
          `${appliedCommissionPercent}%`,
          new Date(payout.paidAt).toLocaleDateString(),
  ...
  const handleRecalculate = (commissionId) => { setRecalculateCommissionId(commissionId); setShowRecalculateModal(true); };
  ...
  const executeRecalculate = async () => {
    ...
    } catch (error) { console.error("Error recalculating:", error); showNotif("Error recalculating commission", "error"); }
  };

  const handleUnflag = (commissionId) => { setUnflagCommissionId(commissionId); setUnflagNotes(""); setShowUnflagModal(true); };
  ...
  const executeUnflag = async () => {
    ...
    } catch (error) { console.error("Error unflagging:", error); showNotif("Error unflagging commission", "error"); }
  };
  ...
  const togglePayoutSelection = (payoutId) => {
    ...
    if (newSelected.has(payoutId)) newSelected.delete(payoutId);
    else newSelected.add(payoutId);
    setSelectedPayouts(newSelected);
  };

  const selectAllInGroup = (group) => {
    const newSelected = new Set(selectedPayouts);
    group.payouts.forEach((payout) => newSelected.add(payout.id));
    setSelectedPayouts(newSelected);
  };

  const formatCurrency = (amount) => new Intl.NumberFormat("en-US", { style: "currency", currency: "USD" }).format(amount || 0);

  const isPaymentDenied = (flagReason) => flagReason && flagReason.startsWith("PAYMENT_DENIED:");
  const parseDenialReason = (flagReason) => isPaymentDenied(flagReason) ? flagReason.replace("PAYMENT_DENIED: ", "") : null;

  const getCommissionDisplayName = (commission) => {
    if (commission.order?.account?.name) return commission.order.account.name;
    if (commission.order?.poNumber) return `PO #${commission.order.poNumber}`;
    return "Order Deleted";
  };

  const uniquePaidSalesReps = [...new Set(recentlyPaid.map(p => p.itemCommission?.commission?.salesPersonName).filter(Boolean))].sort();
  const filteredPaidCommissions = paidFilterSalesRep
    ? recentlyPaid.filter(p => p.itemCommission?.commission?.salesPersonName === paidFilterSalesRep)
    : recentlyPaid;
  ...
  return (
    <>
      ...
      <div style={{ maxWidth: "1400px", margin: "0 auto", padding: "100px 24px 24px" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: "30px", borderBottom: "2px solid #333" }}>
          <div style={{ display: "flex", gap: "10px" }}>
            {["flagged", "pending", "approved", "paid", "orphaned", user.role === "SUPER_ADMIN" && "settings"].filter(Boolean).map((tab) => (
        ...
        {loading ? (
          ...
          <>
            ...
            {activeTab === "flagged" && (
              <div>
                <div style={{ marginBottom: "20px", color: "#999" }}>{flaggedCommissions.length} commissions need attention</div>
                {flaggedCommissions.map((commission) => (
                  <div key={commission.id} style={{ background: "#1a1a1a", border: isPaymentDenied(commission.flagReason) ? "1px solid #dc2626" : "1px solid #333", borderRadius: "8px", padding: "20px", marginBottom: "16px" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "start" }}>
                      <div style={{ flex: 1 }}>
                        ...
                        {isPaymentDenied(commission.flagReason) && (
                          <div>
                            <div style={{ color: "#dc2626", fontWeight: "600", marginBottom: "8px", fontSize: "15px" }}>Payment Denied</div>
                            <div style={{ color: "#ccc", marginBottom: "8px", fontSize: "14px", lineHeight: "1.5" }}>{parseDenialReason(commission.flagReason)}</div>
                            <div style={{ padding: "12px", backgroundColor: "rgba(220, 38, 38, 0.1)", border: "1px solid rgba(220, 38, 38, 0.3)", borderRadius: "6px", marginTop: "12px" }}>
                              <p style={{ margin: "0", fontSize: "13px", color: "#ef4444" }}>The payout has been reset to WAITING status. It will be retriggered when the item reaches the appropriate stage again, or you can unflag this commission to clear the denial.</p>
                            </div>
                          </div>
                        )}
                        {!isPaymentDenied(commission.flagReason) && (
                      ...
                      <div style={{ display: "flex", gap: "8px", marginLeft: "16px" }}>
                        {(isPaymentDenied(commission.flagReason) || commission.flagReason === "AWAITING_PRICES") && commission.orderId && (
                          <button onClick={() => router.push(`/admin/orders/${commission.orderId}`)} style={{ padding: "8px 16px", background: "#dc2626", color: "white", border: "none", borderRadius: "4px", cursor: "pointer" }}>View Order</button>
                        )}
                        {commission.flagReason === "PRICE_CHANGED" && user.role === "SUPER_ADMIN" && (
                          <button onClick={() => handleRecalculate(commission.id)} style={{ padding: "8px 16px", background: "#f59e0b", color: "white", border: "none", borderRadius: "4px", cursor: "pointer" }}>Recalculate</button>
                        )}
                        <button onClick={() => handleUnflag(commission.id)} style={{ padding: "8px 16px", background: "#666", color: "white", border: "none", borderRadius: "4px", cursor: "pointer" }}>Unflag</button>
                      </div>
                    </div>
                  </div>
                ))}
                {flaggedCommissions.length === 0 && <div style={{ padding: "40px", textAlign: "center", color: "#666" }}>No flagged commissions</div>}
              </div>
            )}
            ...
            {activeTab === "pending" && (
              <div>
                <div style={{ marginBottom: "20px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  ...
                  <div>Total pending: {formatCurrency(payoutGroups.reduce((sum, group) => sum + group.total, 0))}</div>
                </div>
                {payoutGroups.map((group) => (
                  <div key={group.salesPerson} style={{ background: "#1a1a1a", borderRadius: "8px", border: "1px solid #333", marginBottom: "20px", overflow: "hidden" }}>
                    <div onClick={() => toggleGroup(group.salesPerson)} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "20px", cursor: "pointer", background: expandedGroups.has(group.salesPerson) ? "#252525" : "transparent" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: "15px" }}>
                        <div style={{ width: "40px", height: "40px", borderRadius: "50%", background: "linear-gradient(135deg, #667eea 0%, #764ba2 100%)", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: "bold", color: "white" }}>
                    ...
                    {expandedGroups.has(group.salesPerson) && (
                      <div style={{ padding: "0 20px 20px" }}>
                        <div style={{ marginBottom: "16px" }}>
                          <button onClick={() => selectAllInGroup(group)} style={{ padding: "8px 16px", background: "#333", color: "white", border: "none", borderRadius: "4px", cursor: "pointer", marginRight: "8px" }}>Select All</button>
                        </div>
                        <table style={{ width: "100%", tableLayout: "fixed" }}>
                          <colgroup>
                            <col style={{ width: "40px" }} /><col style={{ width: "27%" }} /><col style={{ width: "27%" }} />
                            {stageSettings.map((_, index) => <col key={index} style={{ width: "50px" }} />)}
                          ...
                          <thead>
                            <tr style={{ borderBottom: "1px solid #333" }}>
                              ...
                              {stageSettings.map((stageSetting, index) => (
                                <th key={stageSetting.stage} style={{ padding: "4px 2px", textAlign: "center", fontSize: "11px", color: "#fff" }} title={`${stageSetting.stage} (${stageSetting.percentage}%)`}>P{index + 1}</th>
                              ))}
                          ...
                          <tbody>
                            {group.payouts.map((payout) => (
                              <tr key={payout.id} style={{ borderBottom: "1px solid #333" }}>
                                <td style={{ padding: "4px", textAlign: "center" }}><input type="checkbox" checked={selectedPayouts.has(payout.id)} onChange={() => togglePayoutSelection(payout.id)} /></td>
                                ...
                                <td style={{ padding: "8px 4px", color: "#ccc", fontWeight: "bold", textAlign: "right", fontSize: "13px" }}>
                                  ...
                                  {payout.itemCommission?.allocatedDiscount > 0 && stageSettings.length > 0 && (
                                    <span style={{ color: "#dc2626", fontSize: "11px", marginLeft: "4px" }}>({formatCurrency(((payout.itemCommission.itemSubtotal || 0) / stageSettings.length) - (payout.itemCommission.allocatedDiscount / stageSettings.length))})</span>
                                  )}
                                </td>
                ...
                {payoutGroups.length === 0 && <div style={{ padding: "40px", textAlign: "center", color: "#666" }}>No pending approvals</div>}
              </div>
            )}
              ...
              <div>
                <div style={{ marginBottom: "20px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  ...
                  <div style={{ display: "flex", gap: "10px", alignItems: "center" }}>
                    <select value={paymentMethod} onChange={(e) => setPaymentMethod(e.target.value)} style={{ padding: "8px", background: "#1a1a1a", color: "white", border: "1px solid #333", borderRadius: "4px" }}>
                    ...
                    <input type="text" placeholder="Payment notes (optional)" value={paymentNotes} onChange={(e) => setPaymentNotes(e.target.value)} style={{ padding: "8px", background: "#1a1a1a", color: "white", border: "1px solid #333", borderRadius: "4px", width: "200px" }} />
                  </div>
                </div>
                <div style={{ background: "#1a1a1a", borderRadius: "8px", border: "1px solid #333", overflow: "hidden" }}>
                  <table style={{ width: "100%", tableLayout: "fixed" }}>
                    <colgroup>
                      <col style={{ width: "40px" }} /><col style={{ width: "20%" }} /><col style={{ width: "20%" }} /><col style={{ width: "13%" }} />
                    ...
                    <thead>
                      <tr style={{ background: "#252525", borderBottom: "1px solid #333" }}>
                        <th style={{ padding: "4px", textAlign: "center", fontSize: "11px" }}><input type="checkbox" onChange={(e) => { if (e.target.checked) setSelectedPayouts(new Set(approvedPayouts.map(p => p.id))); else setSelectedPayouts(new Set()); }} /></th>
                      ...
                      {approvedPayouts.map((payout) => {
                        ...
                        return (
                          <tr key={payout.id} style={{ borderBottom: "1px solid #333" }}>
                            ...
                            <td style={{ padding: "8px", color: "#ccc", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{payout.itemCommission?.productCode || "N/A"}</td>
                            <td style={{ padding: "8px", color: "#ccc", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{payout.itemCommission.commission.salesPersonName}</td>
                            <td style={{ padding: "8px 4px", color: "#999", textAlign: "center", fontSize: "11px" }}>P{paymentNumber > 0 ? paymentNumber : "?"}</td>
                ...
                {approvedPayouts.length === 0 && <div style={{ padding: "40px", textAlign: "center", color: "#666" }}>No approved payouts ready for payment</div>}
              </div>
            )}
            ...
            {activeTab === "paid" && (
              <div>
                <div style={{ marginBottom: "20px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  ...
                  <div style={{ display: "flex", gap: "10px", alignItems: "center" }}>
                    <select value={paidFilterSalesRep} onChange={(e) => { setPaidFilterSalesRep(e.target.value); setSelectedPayouts(new Set()); }} style={{ padding: "8px", background: "#1a1a1a", color: "white", border: "1px solid #333", borderRadius: "4px", minWidth: "180px" }}>
                      <option value="">All Sales Reps</option>
                      {uniquePaidSalesReps.map((rep) => <option key={rep} value={rep}>{rep}</option>)}
                    </select>
                    {paidFilterSalesRep && (
                      <button onClick={() => { setPaidFilterSalesRep(""); setSelectedPayouts(new Set()); }} style={{ padding: "8px 12px", background: "#333", color: "#999", border: "none", borderRadius: "4px", cursor: "pointer", fontSize: "12px" }}>Clear</button>
                    )}
                  </div>
                </div>
                <div style={{ background: "#1a1a1a", borderRadius: "8px", border: "1px solid #333", overflow: "hidden" }}>
                  <table style={{ width: "100%", tableLayout: "fixed" }}>
                    <colgroup>
                      <col style={{ width: "40px" }} /><col style={{ width: "15%" }} /><col style={{ width: "18%" }} /><col style={{ width: "12%" }} />
                      <col style={{ width: "50px" }} /><col style={{ width: "90px" }} /><col style={{ width: "65px" }} /><col style={{ width: "80px" }} />
                      <col style={{ width: "10%" }} /><col style={{ width: "50px" }} />
                    </colgroup>
                    <thead>
                      <tr style={{ background: "#252525", borderBottom: "1px solid #333" }}>
                        <th style={{ padding: "4px", textAlign: "center", fontSize: "11px" }}>
                          <input type="checkbox" checked={filteredPaidCommissions.length > 0 && filteredPaidCommissions.every(p => selectedPayouts.has(p.id))} onChange={(e) => { if (e.target.checked) setSelectedPayouts(new Set(filteredPaidCommissions.map(p => p.id))); else setSelectedPayouts(new Set()); }} />
                        </th>
                        <th style={{ padding: "8px", textAlign: "left", color: "#999", fontSize: "12px" }}>Customer Name</th>
                        <th style={{ padding: "8px", textAlign: "left", color: "#999", fontSize: "12px" }}>Item Name</th>
                        <th style={{ padding: "8px", textAlign: "left", color: "#999", fontSize: "12px" }}>Sales Rep</th>
                      ...
                      {filteredPaidCommissions.map((payout) => {
                        ...
                        return (
                          <tr key={payout.id} style={{ borderBottom: "1px solid #333" }}>
                            ...
                            <td style={{ padding: "8px 4px", color: "#999", fontSize: "11px" }}>{payout.paymentMethod || "N/A"}</td>
                ...
                {filteredPaidCommissions.length === 0 && <div style={{ padding: "40px", textAlign: "center", color: "#666" }}>{paidFilterSalesRep ? "No paid commissions for selected sales rep" : "No payment history available"}</div>}
              </div>
            )}
              ...
              <div>
                ...
                {orphanedCommissions.map((commission) => (
                  <div key={commission.id} style={{ background: "#1a1a1a", border: "1px solid #333", borderRadius: "8px", padding: "20px", marginBottom: "16px" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "start" }}>
                      <div>
                        <h3 style={{ color: "#f59e0b", marginBottom: "8px" }}>PO #{commission.order?.poNumber || "Unknown"} - {commission.salesPersonName}</h3>
                        <div style={{ color: "#999", marginBottom: "4px" }}>Commission: {formatCurrency(commission.totalCommissionAmount)}</div>
                        <div style={{ color: "#999", fontSize: "14px" }}>Status: {commission.status}</div>
                        <div style={{ marginTop: "8px" }}>
                          {commission.itemCommissions?.map((itemComm) => itemComm.payouts?.map((payout) => (
                            <div key={payout.id} style={{ color: "#666", fontSize: "13px" }}>{payout.stage}: {formatCurrency(payout.amount)} ({payout.status})</div>
                          )))}
                        </div>
                      </div>
                      <div style={{ display: "flex", gap: "8px" }}>
                        {user.role === "SUPER_ADMIN" && (
                          <button onClick={() => handleDeleteOrphanedCommission(commission.id)} style={{ padding: "8px 16px", background: "#dc2626", color: "white", border: "none", borderRadius: "4px", cursor: "pointer" }}>Delete Commission</button>
                        )}
                        <button style={{ padding: "8px 16px", background: "#666", color: "white", border: "none", borderRadius: "4px", cursor: "pointer" }}>Keep for Records</button>
                      </div>
                    </div>
                  </div>
                ))}
                {orphanedCommissions.length === 0 && <div style={{ padding: "40px", textAlign: "center", color: "#666" }}>No orphaned commissions</div>}
              </div>
            )}
          </>
        )}
        ...
        {/* Bulk Actions Bar */}
        {(activeTab === "pending" || activeTab === "approved" || activeTab === "paid") && selectedPayouts.size > 0 && (
          <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, background: "#1a1a1a", padding: "20px", borderTop: "2px solid #333", display: "flex", gap: "10px", justifyContent: "center", alignItems: "center" }}>
            <span style={{ color: "#999" }}>{selectedPayouts.size} selected</span>
            {activeTab === "pending" && (
              <button onClick={handleBulkApprove} style={{ padding: "10px 20px", background: "#10b981", color: "white", border: "none", borderRadius: "4px", cursor: "pointer", fontWeight: "600" }}>Approve Selected ({selectedPayouts.size})</button>
            )}
            {activeTab === "approved" && (
              <button onClick={handleBulkPay} style={{ padding: "10px 20px", background: "#10b981", color: "white", border: "none", borderRadius: "4px", cursor: "pointer", fontWeight: "600" }}>Mark Selected as Paid ({selectedPayouts.size})</button>
            )}
            {activeTab === "paid" && (
              <button onClick={generateReportFromSelected} style={{ padding: "10px 20px", background: "#dc2626", color: "white", border: "none", borderRadius: "4px", cursor: "pointer", fontWeight: "600" }}>Generate Report from Selected ({selectedPayouts.size})</button>
            )}
            <button onClick={() => setSelectedPayouts(new Set())} style={{ padding: "10px 20px", background: "#666", color: "white", border: "none", borderRadius: "4px", cursor: "pointer" }}>Clear Selection</button>
          </div>
        )}
      </div>
      ...
      {showRejectModal && (
        <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, backgroundColor: "rgba(0, 0, 0, 0.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }} onClick={() => setShowRejectModal(false)}>
          <div style={{ backgroundColor: "#1f1f1f", border: "1px solid #404040", borderRadius: "8px", padding: "2rem", maxWidth: "500px", width: "90%", boxShadow: "0 4px 20px rgba(0, 0, 0, 0.5)" }} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ display: "flex", gap: "1rem", justifyContent: "flex-end", marginTop: "1.5rem" }}>
              <button onClick={() => setShowRejectModal(false)} style={{ background: "#2d2d2d", color: "#fff", border: "1px solid #404040", padding: "0.5rem 1.5rem", borderRadius: "6px", cursor: "pointer", fontSize: "14px" }}>Cancel</button>
              <button onClick={executeReject} disabled={rejectionReason.trim().length < 10} style={{ backgroundColor: "#dc2626", color: "white", border: "none", padding: "0.5rem 1.5rem", borderRadius: "6px", cursor: rejectionReason.trim().length < 10 ? "not-allowed" : "pointer", fontSize: "14px", opacity: rejectionReason.trim().length < 10 ? 0.5 : 1 }}>Deny Payment</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showUnflagModal && (
        <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, backgroundColor: "rgba(0, 0, 0, 0.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }} onClick={() => setShowUnflagModal(false)}>
          <div style={{ backgroundColor: "#1f1f1f", border: "1px solid #404040", borderRadius: "8px", padding: "2rem", maxWidth: "500px", width: "90%", boxShadow: "0 4px 20px rgba(0, 0, 0, 0.5)" }} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ display: "flex", gap: "1rem", justifyContent: "flex-end", marginTop: "1.5rem" }}>
              <button onClick={() => setShowUnflagModal(false)} style={{ background: "#2d2d2d", color: "#fff", border: "1px solid #404040", padding: "0.5rem 1.5rem", borderRadius: "6px", cursor: "pointer", fontSize: "14px" }}>Cancel</button>
              <button onClick={executeUnflag} style={{ backgroundColor: "#10b981", color: "white", border: "none", padding: "0.5rem 1.5rem", borderRadius: "6px", cursor: "pointer", fontSize: "14px" }}>Unflag Commission</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showDeleteOrphanModal && (
        <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, backgroundColor: "rgba(0, 0, 0, 0.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }} onClick={() => setShowDeleteOrphanModal(false)}>
          <div style={{ backgroundColor: "#1f1f1f", border: "1px solid #404040", borderRadius: "8px", padding: "2rem", maxWidth: "500px", width: "90%", boxShadow: "0 4px 20px rgba(0, 0, 0, 0.5)" }} onClick={(e) => e.stopPropagation()}>
            <h3 style={{ fontSize: "20px", fontWeight: "600", color: "#fff", marginTop: 0, marginBottom: "1rem" }}>🗑️ Delete Orphaned Commission</h3>
            ...
            <div style={{ display: "flex", gap: "1rem", justifyContent: "flex-end", marginTop: "1.5rem" }}>
              <button onClick={() => setShowDeleteOrphanModal(false)} style={{ background: "#2d2d2d", color: "#fff", border: "1px solid #404040", padding: "0.5rem 1.5rem", borderRadius: "6px", cursor: "pointer", fontSize: "14px" }}>Cancel</button>
              <button onClick={executeDeleteOrphan} style={{ backgroundColor: "#dc2626", color: "white", border: "none", padding: "0.5rem 1.5rem", borderRadius: "6px", cursor: "pointer", fontSize: "14px" }}>Delete Commission</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showRecalculateModal && (
        <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, backgroundColor: "rgba(0, 0, 0, 0.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }} onClick={() => setShowRecalculateModal(false)}>
          <div style={{ backgroundColor: "#1f1f1f", border: "1px solid #404040", borderRadius: "8px", padding: "2rem", maxWidth: "500px", width: "90%", boxShadow: "0 4px 20px rgba(0, 0, 0, 0.5)" }} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ display: "flex", gap: "1rem", justifyContent: "flex-end", marginTop: "1.5rem" }}>
              <button onClick={() => setShowRecalculateModal(false)} style={{ background: "#2d2d2d", color: "#fff", border: "1px solid #404040", padding: "0.5rem 1.5rem", borderRadius: "6px", cursor: "pointer", fontSize: "14px" }}>Cancel</button>
              <button onClick={executeRecalculate} style={{ backgroundColor: "#f59e0b", color: "white", border: "none", padding: "0.5rem 1.5rem", borderRadius: "6px", cursor: "pointer", fontSize: "14px" }}>Recalculate</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showUnapproveModal && (
        <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, backgroundColor: "rgba(0, 0, 0, 0.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }} onClick={() => setShowUnapproveModal(false)}>
          <div style={{ backgroundColor: "#1f1f1f", border: "1px solid #404040", borderRadius: "8px", padding: "2rem", maxWidth: "500px", width: "90%", boxShadow: "0 4px 20px rgba(0, 0, 0, 0.5)" }} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ display: "flex", gap: "1rem", justifyContent: "flex-end", marginTop: "1.5rem" }}>
              <button onClick={() => setShowUnapproveModal(false)} style={{ background: "#2d2d2d", color: "#fff", border: "1px solid #404040", padding: "0.5rem 1.5rem", borderRadius: "6px", cursor: "pointer", fontSize: "14px" }}>Cancel</button>
              <button onClick={executeUnapprove} style={{ backgroundColor: "#f59e0b", color: "white", border: "none", padding: "0.5rem 1.5rem", borderRadius: "6px", cursor: "pointer", fontSize: "14px" }}>Undo Approval</button>
            </div>
          </div>
        </div>
      )}
      ...
      {/* Unpay Payment Modal */}
      {showUnpayModal && (
        <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, backgroundColor: "rgba(0, 0, 0, 0.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }} onClick={() => setShowUnpayModal(false)}>
          <div style={{ backgroundColor: "#1f1f1f", border: "1px solid #404040", borderRadius: "8px", padding: "2rem", maxWidth: "500px", width: "90%", boxShadow: "0 4px 20px rgba(0, 0, 0, 0.5)" }} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ display: "flex", gap: "1rem", justifyContent: "flex-end", marginTop: "1.5rem" }}>
              <button onClick={() => setShowUnpayModal(false)} style={{ background: "#2d2d2d", color: "#fff", border: "1px solid #404040", padding: "0.5rem 1.5rem", borderRadius: "6px", cursor: "pointer", fontSize: "14px" }}>Cancel</button>
              <button onClick={executeUnpay} style={{ backgroundColor: "#f59e0b", color: "white", border: "none", padding: "0.5rem 1.5rem", borderRadius: "6px", cursor: "pointer", fontSize: "14px" }}>Undo Payment</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showPdfModal && (
        <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(0, 0, 0, 0.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }} onClick={() => setShowPdfModal(false)}>
          <div style={{ background: "#1a1a1a", padding: "30px", borderRadius: "12px", maxWidth: "500px", width: "90%", border: "1px solid #333" }} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ display: "flex", gap: "12px", justifyContent: "flex-end" }}>
              <button onClick={() => { setShowPdfModal(false); setPdfAgent(""); setPdfStartDate(""); setPdfEndDate(""); }} style={{ background: "#2a2a2a", color: "#999", border: "1px solid #444", padding: "10px 20px", borderRadius: "6px", cursor: "pointer", fontSize: "14px" }}>Cancel</button>
              <button onClick={generatePdfReport} style={{ background: "#dc2626", color: "white", border: "none", padding: "10px 20px", borderRadius: "6px", cursor: "pointer", fontSize: "14px", fontWeight: "600" }}>Generate PDF</button>
            </div>
          </div>
        </div>
      )}
