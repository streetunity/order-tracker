...
// =============================
// Commission Helper Functions (Global)
// =============================
// Create commission helper functions that will be used by orders and items routes
async function calculateCommissionForOrder(prisma, orderId) {
  try {
...
async function recalculateCommissionIfPriceChanged(prisma, orderId) {
  try {
    const commission = await prisma.commission.findFirst({
      where: { orderId }
    });
...
async function checkCommissionPayoutTrigger(prisma, orderId, oldStage, newStage) {
  try {
...
app.use('/commission-settings', commissionSettingsRouter);
