...
const DEFAULTS = {
  ...
  phone: '', email: 'info@stealthlaser.com', website: 'https://smt-orders.com',
...
export function createInvoicingSettingsRouter(prisma) {
  ...
  router.get('/', adminGuard, async (req, res) => {
    try {
      ...
      if (!settings) {
        settings = await prisma.invoicingSettings.create({ data: { ...DEFAULTS } });
      }
      res.json(settings);
    } catch (error) {
  ...
  router.put('/', adminGuard, async (req, res) => {
    try {
      ...
      if (settings) {
        settings = await prisma.invoicingSettings.update({ where: { id: settings.id }, data });
      } else {
        settings = await prisma.invoicingSettings.create({ data: { companyName: DEFAULTS.companyName, ...data } });
      }
