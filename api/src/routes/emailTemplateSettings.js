...
// Sample payout details table used in preview / test-send
...
const DEFAULT_TEMPLATES = {
  ...
  commission_notification: {
    ...
    variables: [
      ...
      { name: 'payoutDetails',  description: 'HTML table: Item | Customer | Stage | Amount (one row per payout, only this agent\'s payouts)' },
...
export function createEmailTemplateSettingsRouter(prisma) {
  ...
  router.get('/', adminGuard, async (req, res) => {
    try {
      ...
      const templates = Object.entries(DEFAULT_TEMPLATES).map(([key, defaultTpl]) => {
        ...
        if (dbTpl) {
          return { ...defaultTpl, key: dbTpl.templateKey, subject: dbTpl.subject, bodyContent: dbTpl.bodyContent, closingContent: dbTpl.closingContent || '', footerContent: dbTpl.footerContent || '', isCustomized: true, lastUpdatedAt: dbTpl.updatedAt, lastUpdatedBy: dbTpl.updatedByName };
        }
  ...
  router.get('/:key', adminGuard, async (req, res) => {
    try {
      ...
      if (dbTpl) {
        return res.json({ ...defaultTpl, key: dbTpl.templateKey, subject: dbTpl.subject, bodyContent: dbTpl.bodyContent, closingContent: dbTpl.closingContent || '', footerContent: dbTpl.footerContent || '', isCustomized: true, lastUpdatedAt: dbTpl.updatedAt, lastUpdatedBy: dbTpl.updatedByName });
      }
  ...
  router.post('/test-send', adminGuard, async (req, res) => {
    try {
      ...
      const sampleData = {
        ...
        trackingUrl: '#', unsubscribeUrl: '#',
        // Commission vars
        agentName: req.user.name, type: 'Approved',
        amount: '625.00', payoutCount: '2',
        itemName: 'SL-3015', payoutStage: 'SHIPPING',
        payoutDetails: SAMPLE_PAYOUT_DETAILS,
        commissionsUrl: 'https://smt-orders.com/my-commissions',
