...
export function createInvoicesRouter(prisma) {
  ...
  router.post('/', requireInvoicingPermission('CREATE_INVOICE'), async (req, res) => {
    try {
      const {
        customerId, invoiceDate, dueDate, paymentTerms,
        items, taxRate, discountType, discountValue, shippingAmount,
        paymentType, paymentScheduleType, depositRequired,
        orderCreationTrigger, notes, internalNotes, termsConditions, estimateId
      } = req.body;
      ...
      const totals = calculateTotals(itemsToCreate, taxRate || 0, discountType, discountValue, shippingAmount || 0);
      ...
      if (paymentScheduleType && paymentScheduleType !== 'NONE') {
        ...
        if (scheduleItems.length > 0) {
          ...
          // This is used by the order auto-creation trigger to know when the
          // deposit has been received, and by depositPaid tracking.
          await prisma.invoice.update({
            where: { id: invoice.id },
            data:  { depositRequired: scheduleItems[0].amount }
          });
        }
      }
      ...
      if (estimateId) {
        await prisma.estimate.update({
          where: { id: estimateId },
          data:  { status: 'CONVERTED', convertedToInvoiceId: invoice.id, convertedAt: new Date() }
        });
      }
  ...
  router.post('/from-estimate/:estimateId', requireInvoicingPermission('CONVERT_ESTIMATE_TO_INVOICE'), async (req, res) => {
    try {
      ...
      if (estimate.status === 'CONVERTED') return res.status(400).json({ error: 'Estimate has already been converted to invoice' });
  ...
  router.put('/:id', requireInvoicingPermission('EDIT_INVOICE'), async (req, res) => {
    try {
      const existing = await prisma.invoice.findUnique({ where: { id: req.params.id }, include: { items: true } });
      if (!existing) return res.status(404).json({ error: 'Invoice not found' });
      if (req.user.role === 'AGENT' && existing.createdById !== req.user.id) return res.status(403).json({ error: 'Access denied' });

      const { dueDate, paymentTerms, taxRate, discountType, discountValue, shippingAmount, status, notes, internalNotes, termsConditions } = req.body;
  ...
  router.post('/:id/items', requireInvoicingPermission('EDIT_INVOICE'), async (req, res) => {
    try {
      ...
      if (!invoice) return res.status(404).json({ error: 'Invoice not found' });
      if (req.user.role === 'AGENT' && invoice.createdById !== req.user.id) return res.status(403).json({ error: 'Access denied' });
      ...
      if (productId) {
        ...
        itemData = { invoiceId: invoice.id, productId: product.id, name: product.name, description: product.description, sku: product.sku, quantity: qty, unitPrice: product.price, unitCost: product.cost, amount: qty * product.price, taxable: product.taxable, sortOrder: maxSortOrder + 1 };
      } else {
        if (!customName) return res.status(400).json({ error: 'Product ID or custom name is required' });
        const qty = quantity || 1; const price = customPrice || 0;
        itemData = { invoiceId: invoice.id, name: customName, description: customDescription, quantity: qty, unitPrice: price, unitCost: customCost, amount: qty * price, taxable: taxable !== undefined ? taxable : true, sortOrder: maxSortOrder + 1 };
      }

      const newItem = await prisma.invoiceItem.create({ data: itemData });
      const allItems = [...invoice.items, newItem];
      const totals = calculateTotals(allItems, invoice.taxRate, invoice.discountType, invoice.discountValue, invoice.shippingAmount);

      const updatedInvoice = await prisma.invoice.update({
        where: { id: invoice.id },
        data: { subtotal: totals.subtotal, taxAmount: totals.taxAmount, discountAmount: totals.discountAmount, total: totals.total, balanceDue: totals.total - invoice.amountPaid, totalCost: totals.totalCost, marginAmount: totals.marginAmount, marginPercent: totals.marginPercent },
        include: { customer: true, items: { orderBy: { sortOrder: 'asc' } }, payments: { orderBy: { createdAt: 'desc' } }, paymentSchedule: { orderBy: { sortOrder: 'asc' } } }
      });
  ...
  router.patch('/:id/items/:itemId', requireInvoicingPermission('EDIT_INVOICE'), async (req, res) => {
    try {
      ...
      updateData.quantity = newQty; updateData.unitPrice = newPrice; updateData.amount = newQty * newPrice;

      const updatedItem = await prisma.invoiceItem.update({ where: { id: req.params.itemId }, data: updateData });
      const allItems = invoice.items.map(i => i.id === req.params.itemId ? updatedItem : i);
      const totals = calculateTotals(allItems, invoice.taxRate, invoice.discountType, invoice.discountValue, invoice.shippingAmount);
  ...
  router.delete('/:id/items/:itemId', requireInvoicingPermission('EDIT_INVOICE'), async (req, res) => {
    try {
      ...
      const totals = calculateTotals(remainingItems, invoice.taxRate, invoice.discountType, invoice.discountValue, invoice.shippingAmount);
  ...
  router.post('/:id/payments', requireInvoicingPermission('RECORD_PAYMENT'), async (req, res) => {
    try {
      ...
      const { amount, paymentMethod, paymentDate, checkNumber, wireReference, referenceNumber, notes, scheduleItemId } = req.body;
      ...
      if (depositPaid && !invoice.convertedToOrder) {
        try {
          ...
          const invoiceForOrder = await prisma.invoice.findUnique({ where: { id: invoice.id }, include: { customer: true, items: true } });
  ...
  router.post('/:id/recalculate', requireInvoicingPermission('EDIT_INVOICE'), async (req, res) => {
    try {
      ...
      const updateData = {
        subtotal: totals.subtotal, taxAmount: totals.taxAmount, discountAmount: totals.discountAmount,
        total: totals.total, balanceDue: totals.total - invoice.amountPaid,
        totalCost: totals.totalCost, marginAmount: totals.marginAmount, marginPercent: totals.marginPercent
      };
      if (taxRate        !== undefined) updateData.taxRate        = taxRate;
      if (discountType   !== undefined) updateData.discountType   = discountType;
      if (discountValue  !== undefined) updateData.discountValue  = discountValue;
      ...
      const updatedInvoice = await prisma.invoice.update({
        where: { id: invoice.id }, data: updateData,
        include: { customer: true, items: { orderBy: { sortOrder: 'asc' } }, payments: { orderBy: { createdAt: 'desc' } }, paymentSchedule: { orderBy: { sortOrder: 'asc' } } }
      });
  ...
  router.post('/:id/void', requireInvoicingPermission('VOID_INVOICE'), async (req, res) => {
    try {
      const invoice = await prisma.invoice.findUnique({ where: { id: req.params.id } });
      if (!invoice) return res.status(404).json({ error: 'Invoice not found' });
      if (invoice.amountPaid > 0) return res.status(400).json({ error: 'Cannot void invoice with payments. Refund payments first.' });

      const updated = await prisma.invoice.update({
        where: { id: req.params.id },
        data:  { status: 'VOID' },
        include: { customer: true, items: { orderBy: { sortOrder: 'asc' } }, payments: { orderBy: { createdAt: 'desc' } } }
      });
  ...
  router.post('/:id/payment-schedule', requireInvoicingPermission('EDIT_INVOICE'), async (req, res) => {
    try {
      const invoice = await prisma.invoice.findUnique({ where: { id: req.params.id }, include: { paymentSchedule: true } });
      if (!invoice) return res.status(404).json({ error: 'Invoice not found' });
      if (req.user.role === 'AGENT' && invoice.createdById !== req.user.id) return res.status(403).json({ error: 'Access denied' });
      ...
      if (invoice.paymentSchedule.length > 0) {
        await prisma.invoicePaymentSchedule.deleteMany({ where: { invoiceId: invoice.id } });
      }
  ...
  // NOTE: PDF generation endpoints moved to invoicePdf.js:
